...
import 'package:erp_mobile_cnplus/features/inventory/delivery_note/presentation/widgets/delivery_note_detail_tabs.dart';
import 'package:erp_mobile_cnplus/features/inventory/delivery_note/presentation/widgets/delivery_note_tracking_dialog.dart';
...
class DeliveryNoteDetailScreen extends StatefulWidget {
  ...
  const DeliveryNoteDetailScreen({super.key, required this.encryption});
...
class _DeliveryNoteDetailScreenState extends State<DeliveryNoteDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final d = ctrl.detail;
    if (d?.isWaitingApproval == true) ctrl.loadSteps(d!.idDeliveryNote);
  }
  ...
  Future<void> _handleDelete() async {
    ...
    final ok = await _confirmDialog(
      'Delete Draft',
      'Delete this delivery note draft?',
      confirmLabel: 'Delete',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.remove(d.idDeliveryNote);
    if (!mounted) return;
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog('Confirm Delivery Note', 'Confirm this delivery note?', confirmLabel: 'Confirm');
    if (ok != true || !mounted) return;

    final f = DeliveryNoteFormModel.fromDetail(d);
  ...
  Future<void> _handleCancel() async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Cancel Delivery Note', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        content: SizedBox(
  ...
  double _trackingTotal(DeliveryNoteItem item) =>
  ...
  Future<void> _handleValidate() async {
    ...
    for (final item in d.items) {
      ...
      final defaultQty = hasTracking ? _trackingTotal(item) : (item.deliveredQty ?? item.demandQty);
      qtyCtrls[item.idDeliveryNoteItem] = TextEditingController(text: defaultQty.toStringAsFixed(2));
    }

    final confirmed = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Validate Delivery Note', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        content: SizedBox(
          width: double.maxFinite,
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(
                  'Enter the quantity actually delivered for each product.',
                  style: GoogleFonts.poppins(fontSize: 12, color: colorGrey),
                ),
                const SizedBox(height: 12),
                ...d.items.map((item) {
                  ...
                  return Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextField(
                          controller: qtyCtrls[item.idDeliveryNoteItem],
    ...
    final items = <DeliveryNoteFormItem>[];
    for (final item in d.items) {
      final raw = qtyCtrls[item.idDeliveryNoteItem]!.text.replaceAll(',', '');
      final delivered = double.tryParse(raw) ?? 0;

      if (delivered > item.demandQty) {
        _snack('${item.productName ?? 'Product'}: delivered qty cannot exceed demand (${item.demandQty.toStringAsFixed(2)})');
        return;
      }

      items.add(DeliveryNoteFormItem(
  ...
  Future<void> _submitValidate(
    ...
    if (allowBackorder == null) {
      ...
      for (int i = 0; i < items.length; i++) {
        if ((items[i].deliveredQty) < d.items[i].demandQty) {
      ...
      if (shortItems.isNotEmpty) {
        final wantsBackorder = await _confirmDialog(
          'Backorder',
          'Delivered quantity is less than demand for:\n${shortItems.join(', ')}\n\n'
  ...
  Future<void> _handleOpenTracking(DeliveryNoteDetailModel d, int index) async {
    final ctrl = context.read<DeliveryNoteController>();
    ...
    final result = await showDeliveryNoteTrackingDialog(
      context: context,
      item: item,
    ...
    final ok = await ctrl.saveTracking(
      idDeliveryNoteItem: item.idDeliveryNoteItem,
  ...
  Future<void> _handleReturn() async {
    ...
    final ok = await _confirmDialog(
      'Create Return',
      'Create a return delivery note from this document?',
  ...
  Future<void> _handleApprove() async {
    ...
    final ok = await _confirmDialog(
      'Approve',
      'Approve this delivery note?',
      confirmLabel: 'Approve',
      confirmColor: const Color(0xFF2E7D32),
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.approve(d.idDeliveryNote);
    if (!mounted) return;
  ...
  Future<void> _handleReject() async {
    ...
    final ok = await _confirmDialog(
      'Reject',
      'Reject this delivery note?',
      confirmLabel: 'Reject',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.reject(d.idDeliveryNote);
    if (!mounted) return;
  ...
  Future<void> _handleViewSteps() async {
    ...
    await ctrl.loadSteps(d.idDeliveryNote);
    if (!mounted) return;
  ...
  void _handleReturnFromForm(DeliveryNoteController ctrl) {
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 1,
        actions: [
          Consumer<DeliveryNoteController>(
            builder: (_, ctrl, __) {
              final d = ctrl.detail;
                  ...
                  if (d.canEdit)
                    IconButton(
                      ...
                      onPressed: () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => DeliveryNoteFormScreen(encryption: _enc)),
      ...
      body: Consumer<DeliveryNoteController>(
        builder: (_, ctrl, __) {
              ...
              Column(
                children: [
                  ...
                  if (d.isDone && !d.hasReturn)
                    _bottomBar([
                      _primaryBtn('Create Return', Icons.undo_outlined, _handleReturn),
                    ]),
                ],
              ),
              if (ctrl.isSaving)
