...
import 'package:erp_mobile_cnplus/features/inventory/receipt_note/presentation/widgets/receipt_note_list_item.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'package:erp_mobile_cnplus/shared/widgets/pagination_bottom_bar.dart';
import 'receipt_note_detail_screen.dart';
import 'receipt_note_form_screen.dart';

class ReceiptNoteListScreen extends StatefulWidget {
  const ReceiptNoteListScreen({super.key});

  @override
  State<ReceiptNoteListScreen> createState() => _ReceiptNoteListScreenState();
}

class _ReceiptNoteListScreenState extends State<ReceiptNoteListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<ReceiptNoteController>().fetchList(),
    );
  }
  ...
  void _goToForm() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const ReceiptNoteFormScreen()),
    ).then((_) {
      context.read<ReceiptNoteController>().fetchList(status: _selectedStatus);
    });
  }
        ...
        children: [
          Container(
            ...
            child: TextField(
              ...
              onChanged: (v) => context.read<ReceiptNoteController>().search(v),
              decoration: InputDecoration(
                hintText: 'Look for references, vendors or source doc...',
                ...
                suffixIcon: _searchCtrl.text.isNotEmpty
                    ? IconButton(
                        ...
                        onPressed: () {
                          _searchCtrl.clear();
                          context.read<ReceiptNoteController>().clearSearch();
                        },
                      )
                    : null,
                filled: true,
          ...
          Expanded(
            child: Consumer<ReceiptNoteController>(
              builder: (_, ctrl, __) {
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => ReceiptNoteListItem(
                            rn: ctrl.pageItems[i],
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => ReceiptNoteDetailScreen(
  ...
  Widget _buildErrorState(ReceiptNoteController ctrl) {
    return Center(
      child: Padding(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            isSearching ? 'Receipt note not found' : 'No receipt notes yet',
            style: GoogleFonts.poppins(
              fontSize: 18,
