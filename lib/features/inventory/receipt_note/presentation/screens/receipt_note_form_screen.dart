...
import 'package:erp_mobile_cnplus/features/inventory/receipt_note/presentation/widgets/receipt_note_form_fields.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';

class ReceiptNoteFormScreen extends StatefulWidget {
  final String? encryption;

  const ReceiptNoteFormScreen({super.key, this.encryption});

  @override
  State<ReceiptNoteFormScreen> createState() => _ReceiptNoteFormScreenState();
}

class _ReceiptNoteFormScreenState extends State<ReceiptNoteFormScreen> {
  late ReceiptNoteFormModel _form;
  ...
  void initState() {
    super.initState();
    _form = ReceiptNoteFormModel(encryption: widget.encryption);
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        final d = ctrl.detail;
        if (d != null) {
          _form = ReceiptNoteFormModel.fromDetail(d);
        }
      }
    } catch (_) {}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          isEditMode ? 'Edit Receipt Note' : 'Create Receipt Note',
