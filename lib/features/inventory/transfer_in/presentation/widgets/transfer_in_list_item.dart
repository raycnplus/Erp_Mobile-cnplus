...
class TransferInListItem extends StatelessWidget {
  final TransferInModel ti;
  final VoidCallback onTap;

  const TransferInListItem({super.key, required this.ti, required this.onTap});
  ...
  Widget build(BuildContext context) {
    final color = Color(ti.statusColor);
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.download_outlined, color: color, size: 24),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${ti.sourceLocationName ?? '-'} → ${ti.destinationLocationName ?? '-'}',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                    ...
                    Row(
                      children: [
                        Flexible(
                          flex: 2,
                          child: Text(
                            _fmt(ti.scheduledDate),
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: GoogleFonts.poppins(fontSize: 11, color: colorGrey),
                          ),
                        ),
                        if (ti.sourceDocument?.isNotEmpty == true) ...[
