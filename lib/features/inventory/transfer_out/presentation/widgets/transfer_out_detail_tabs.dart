...
import 'transfer_out_tracking_dialog.dart';

class TransferOutDetailTabs extends StatefulWidget {
  final TransferOutDetailModel detail;
  final List<TransferOutFormItem> formItems;
  final bool isEditing;
  final Map<int, TextEditingController> qtyControllers;

  const TransferOutDetailTabs({
    super.key,
  ...
  @override
  State<TransferOutDetailTabs> createState() => _State();
}

class _State extends State<TransferOutDetailTabs> with SingleTickerProviderStateMixin {
  ...
  Widget _buildProductsTab(TransferOutDetailModel d) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        final isShort = item.transferredQty != null && item.transferredQty! < item.demandQty;
                      ...
                      IconButton(
                        onPressed: () {
                          ...
                          final lotSerials = ((entry?['lot_serials'] as List?) ?? [])
                              .map((e) => TOLotSerial.fromJson(Map<String, dynamic>.from(e)))
                              .toList();
                          final usages = ((entry?['usage'] as List?) ?? [])
                              .map((e) => TOTrackingUsage.fromJson(Map<String, dynamic>.from(e)))
                              .toList();
                          showTransferOutTrackingDialog(
                            context: context,
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          widget.isEditing
                              ? TextField(
                                  controller: _ctrlFor(item.mappingKey, item.transferredQty),
                                  ...
                                  onChanged: (v) => item.transferredQty = double.tryParse(v),
                                )
                              : Text(
  ...
  Widget _buildInfoTab(TransferOutDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          _infoCard('Transfer Out Info', Icons.upload_outlined, [
  ...
  Widget _buildOthersTab(TransferOutDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
