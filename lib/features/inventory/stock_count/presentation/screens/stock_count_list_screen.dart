...
import 'package:erp_mobile_cnplus/features/inventory/stock_count/presentation/widgets/stock_count_list_item.dart';
...
import 'stock_count_form_screen.dart';
...
class _StockCountListScreenState extends State<StockCountListScreen> {
  ...
  static const _statuses = ['Draft', 'Confirmed', 'Waiting Approval', 'Validated', 'Cancelled'];

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => context.read<StockCountController>().fetchList());
  }
  ...
  void _goToForm() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => const StockCountFormScreen()))
        .then((_) => context.read<StockCountController>().fetchList(status: _selectedStatus));
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text('Stock Count', style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 20, color: colorTextPrimary)),
            Text('Physical stock count management', style: GoogleFonts.poppins(color: colorTextSubtle, fontSize: 12)),
          ],
        ),
        elevation: 0.5,
            ...
            builder: (_, ctrl, __) => PopupMenuButton<String>(
              icon: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                if (_selectedStatus != null && _selectedStatus!.isNotEmpty)
                  Container(margin: const EdgeInsets.only(left: 2), width: 8, height: 8,
        ...
        children: [
          Container(
            ...
            child: TextField(
              ...
              onChanged: (v) => context.read<StockCountController>().search(v),
              decoration: InputDecoration(
                hintText: 'Look for warehouse or location...',
                ...
                suffixIcon: _searchCtrl.text.isNotEmpty
                    ? IconButton(
                        ...
                        onPressed: () {
                          _searchCtrl.clear();
                          context.read<StockCountController>().clearSearch();
                        },
                      )
                    : null,
                filled: true,
          ...
          Expanded(
            child: Consumer<StockCountController>(
              builder: (_, ctrl, __) {
                final bar = PaginationBottomBar(
                  currentPage: ctrl.currentPage, lastPage: ctrl.lastPage,
                  hasPrev: ctrl.hasPrev, hasNext: ctrl.hasNext,
                  onGoToPage: ctrl.goToPage, onPrev: ctrl.prevPage, onNext: ctrl.nextPage,
                  onCreateTap: _goToForm,
                );

                if (ctrl.isLoadingList) return const Center(child: CircularProgressIndicator(color: colorPrimary));
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text('${ctrl.total} stock counts', style: GoogleFonts.poppins(fontSize: 12, color: colorTextSubtle)),
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => StockCountListItem(
                            sc: ctrl.pageItems[i],
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(builder: (_) => StockCountDetailScreen(encryption: ctrl.pageItems[i].encryption)),
  ...
  Widget _buildErrorState(StockCountController ctrl) {
    return Center(
      child: Padding(
  ...
  Widget _buildEmptyState(bool isSearching) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(isSearching ? Icons.search_off : Icons.fact_check_outlined, size: 80, color: colorGrey),
          const SizedBox(height: 16),
          Text(
            isSearching ? 'Stock count not found' : 'No stock counts yet',
