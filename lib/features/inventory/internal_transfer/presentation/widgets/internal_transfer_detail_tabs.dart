...
class InternalTransferDetailTabs extends StatefulWidget {
  final InternalTransferDetailModel detail;
  final ValueChanged<int>? onOpenTracking;

  const InternalTransferDetailTabs({super.key, required this.detail, this.onOpenTracking});

  @override
  State<InternalTransferDetailTabs> createState() => _State();
}

class _State extends State<InternalTransferDetailTabs> with SingleTickerProviderStateMixin {
  ...
  Widget _buildProductsTab(InternalTransferDetailModel d) {
    if (d.items.isEmpty) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        final hasTransferred = item.transferredQty != null;
        final isShort = hasTransferred && item.transferredQty! < item.demandQty;
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Text(
                          hasTransferred ? '${_fmtNum(item.transferredQty!)} ${item.uomName ?? ''}' : '-',
                          style: GoogleFonts.poppins(
                            fontSize: 13,
  ...
  Widget _buildInfoTab(InternalTransferDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          _infoCard('Internal Transfer Info', Icons.compare_arrows_outlined, [
  ...
  Widget _buildOthersTab(InternalTransferDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
