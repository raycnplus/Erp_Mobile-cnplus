...
Future<List<Map<String, dynamic>>?> showInternalTransferTrackingDialog({
  required BuildContext context,
  required InternalTransferItem item,
...
class _TrackingDialog extends StatefulWidget {
  final InternalTransferItem item;
...
class _TrackingDialogState extends State<_TrackingDialog> {
  ...
  List<ITLotSerial> _availableLots = [];
  ...
  Future<void> _fetchAvailableLots() async {
    ...
    try {
      final res = await ctrl.lotSerialsSorted(
        widget.item.idProduct,
        widget.sourceLocation!,
        idInternalTransferItem: widget.item.idInternalTransferItem,
      );
      final list = (res['lot_serials'] as List? ?? [])
          .map((e) => ITLotSerial.fromJson(e as Map<String, dynamic>))
          .toList();
