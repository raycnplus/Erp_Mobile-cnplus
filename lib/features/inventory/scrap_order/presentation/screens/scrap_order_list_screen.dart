...
import 'package:erp_mobile_cnplus/features/inventory/scrap_order/presentation/widgets/scrap_order_list_item.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'package:erp_mobile_cnplus/shared/widgets/pagination_bottom_bar.dart';
import 'scrap_order_detail_screen.dart';
import 'scrap_order_form_screen.dart';

class ScrapOrderListScreen extends StatefulWidget {
  const ScrapOrderListScreen({super.key});

  @override
  State<ScrapOrderListScreen> createState() => _ScrapOrderListScreenState();
}

class _ScrapOrderListScreenState extends State<ScrapOrderListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<ScrapOrderController>().fetchList(),
    );
  }
  ...
  void _goToForm() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const ScrapOrderFormScreen()),
    ).then((_) {
      context.read<ScrapOrderController>().fetchList(status: _selectedStatus);
    });
  }
        ...
        children: [
          Container(
            ...
            child: TextField(
              ...
              onChanged: (v) => context.read<ScrapOrderController>().search(v),
              decoration: InputDecoration(
                hintText: 'Look for document code or location...',
                ...
                suffixIcon: _searchCtrl.text.isNotEmpty
                    ? IconButton(
                        ...
                        onPressed: () {
                          _searchCtrl.clear();
                          context.read<ScrapOrderController>().clearSearch();
                        },
                      )
                    : null,
                filled: true,
          ...
          Expanded(
            child: Consumer<ScrapOrderController>(
              builder: (_, ctrl, __) {
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text('${ctrl.total} scrap orders', style: GoogleFonts.poppins(fontSize: 12, color: colorTextSubtle)),
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => ScrapOrderListItem(
                            so: ctrl.pageItems[i],
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => ScrapOrderDetailScreen(encryption: ctrl.pageItems[i].encryption),
                              ),
  ...
  Widget _buildErrorState(ScrapOrderController ctrl) {
    return Center(
      child: Padding(
  ...
  Widget _buildEmptyState(bool isSearching) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(isSearching ? Icons.search_off : Icons.delete_sweep_outlined, size: 80, color: colorGrey),
          const SizedBox(height: 16),
          Text(
            isSearching ? 'Scrap order not found' : 'No scrap orders yet',
