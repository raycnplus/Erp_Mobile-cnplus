...
class PurchaseRequestFormFields extends StatefulWidget {
  final PurchaseRequestFormModel formData;
  final PurchaseRequestFormOptions? formOptions;
  final VoidCallback onSave;
  final bool isSaving;

  const PurchaseRequestFormFields({
    super.key,
  ...
  @override
  State<PurchaseRequestFormFields> createState() => _State();
}

class _State extends State<PurchaseRequestFormFields> with SingleTickerProviderStateMixin {
  ...
  void _removeItemCtrls(int i) {
    ...
    for (int j = i + 1; j <= widget.formData.items.length; j++) {
      for (final field in ['demandQty', 'estimatedUnitPrice', 'desc']) {
  ...
  PurchaseRequestFormOptions? get _opts => widget.formOptions;
  ...
  bool _validateAndSync() {
    ...
    if (widget.formData.requestedBy == null) {
      _snack('Please select requested by');
      return false;
    }
    ...
    if (widget.formData.items.isEmpty) {
      _snack('Please add at least one item');
      return false;
    }
  ...
  Future<void> _addItem() async {
    ...
    if (products.isEmpty) {
      _snack('No products available');
      return;
    }

    PRProductOption? selectedProduct;
    PRUomOption? selectedUom;

    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      builder: (_) => StatefulBuilder(
        builder: (ctx, setSt) => AlertDialog(
          title: Text('Add Item', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
          content: SizedBox(
            width: double.maxFinite,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                CustomSearchableDropdown<PRProductOption>(
                ...
                if (selectedProduct != null) ...[
                  ...
                  CustomSearchableDropdown<PRUomOption>(
                    value: selectedUom,
                    items: _opts?.uoms ?? [],
                    itemLabel: (u) => u.name,
    ...
    if (result != null && mounted) {
      final prod = result['product'] as PRProductOption;
      final uom = result['uom'] as PRUomOption;
      setState(() {
        widget.formData.items.add(PurchaseRequestFormItem(
  ...
  List<PRLocationOption> _getFilteredLocations() {
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tab,
            children: [_buildHeaderTab(), _buildItemsTab()],
          ),
        ),
        _buildBottomBar(),
      ],
  ...
  Widget _buildHeaderTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          CustomSearchableDropdown<PRUserOption>(
            key: ValueKey('rb_${widget.formData.requestedBy}_${_opts?.users.length}'),
            value: _opts?.users.where((u) => u.id == widget.formData.requestedBy).firstOrNull,
            items: _opts?.users ?? [],
            itemLabel: (u) => u.name,
            onChanged: (v) => setState(() => widget.formData.requestedBy = v?.id),
            isRequired: true,
            label: 'Requested By',
            clearable: false,
          ),
          const SizedBox(height: 14),
          CustomSearchableDropdown<PRVendorOption>(
            key: ValueKey('vendor_${widget.formData.idVendor}_${_opts?.vendors.length}'),
            value: _opts?.vendors.where((d) => d.id == widget.formData.idVendor).firstOrNull,
            items: _opts?.vendors ?? [],
            itemLabel: (d) => d.name,
          ...
          const SizedBox(height: 14),
          CustomSearchableDropdown<PRWarehouseOption>(
          ...
          const SizedBox(height: 14),
          CustomSearchableDropdown<PRLocationOption>(
          ...
          const SizedBox(height: 14),
          CustomSearchableDropdown<PRPriceListOption>(
          ...
          _datePicker(
            value: widget.formData.requestDate,
            hint: 'Select request date (optional)',
            onPicked: (d) => setState(() => widget.formData.requestDate = d),
            onCleared: () => setState(() => widget.formData.requestDate = null),
          ),
          const SizedBox(height: 14),
          _sectionLabel('Expected Arrival'),
          _datePicker(
            value: widget.formData.expectedArrival,
            hint: 'Select expected arrival (optional)',
            onPicked: (d) => setState(() => widget.formData.expectedArrival = d),
            onCleared: () => setState(() => widget.formData.expectedArrival = null),
          ),
          const SizedBox(height: 14),
          CustomFormInput(
            controller: _noteCtrl,
            label: 'Note',
            hintText: 'Write note or reason for this purchase request (optional)',
            maxLines: 3,
          ),
  ...
  Widget _buildItemsTab() {
    return Column(
      children: [
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      Text('No items added', style: GoogleFonts.poppins(fontSize: 16, color: colorTextSubtle)),
                      const SizedBox(height: 4),
                      Text(
                        'Select destination location first, then add items',
                        style: GoogleFonts.poppins(fontSize: 12, color: colorGrey),
                        textAlign: TextAlign.center,
                      ),
                      const SizedBox(height: 16),
                      ElevatedButton.icon(
                        onPressed: widget.formData.destinationLocation != null ? _addItem : null,
        ...
        if (widget.formData.items.isNotEmpty) _buildTotalFooter(),
      ],
    );
  }
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: _numField(
                    ctrl: _itemCtrl(i, 'estimatedUnitPrice', item.estimatedUnitPrice),
                    label: 'Est. Unit Price (Rp)',
                    onChanged: (v) => setState(() => item.estimatedUnitPrice = v),
                  ),
                ),
              ],
            ),
            if (item.totalEstimatedPrice > 0) ...[
              const Divider(height: 14, color: colorGreyLight),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Total Estimated', style: GoogleFonts.poppins(fontSize: 11, color: colorTextSubtle)),
                  Text(
  ...
  Widget _buildTotalFooter() => Container(
        padding: const EdgeInsets.fromLTRB(16, 10, 16, 12),
        color: colorCard,
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text('Total Estimated', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 14, color: colorTextPrimary)),
            Text(
              'Rp ${_fmtNum(widget.formData.totalPrice)}',
  ...
  Widget _buildBottomBar() => Container(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(
              widget.formData.isEditMode ? 'Update Purchase Request' : 'Save Purchase Request',
        ...
        child: Row(
          children: [
            ...
            if (value != null)
              GestureDetector(
                onTap: onCleared,
                child: const Icon(Icons.close, size: 16, color: colorGrey),
              ),
          ],
        ),
      ),
