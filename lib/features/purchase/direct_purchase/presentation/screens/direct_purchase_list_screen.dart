...
import 'package:erp_mobile_cnplus/features/purchase/direct_purchase/presentation/widgets/direct_purchase_list_item.dart';
...
class DirectPurchaseListScreen extends StatefulWidget {
  const DirectPurchaseListScreen({super.key});

  @override
  State<DirectPurchaseListScreen> createState() => _DirectPurchaseListScreenState();
}

class _DirectPurchaseListScreenState extends State<DirectPurchaseListScreen> {
  final _searchCtrl = TextEditingController();
  String? _selectedStatus;

  static const _statuses = ['Draft', 'Waiting Approval', 'Confirmed', 'Done', 'Cancelled', 'Closed'];

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<DirectPurchaseController>().fetchList(),
    );
  }
  ...
  void _goToForm() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const DirectPurchaseFormScreen()),
    ).then((_) {
      context.read<DirectPurchaseController>().fetchList(status: _selectedStatus);
    });
  }
        ...
        children: [
          Container(
            ...
            child: TextField(
              ...
              onChanged: (v) => context.read<DirectPurchaseController>().search(v),
              decoration: InputDecoration(
                hintText: 'Look for references or vendors...',
                ...
                suffixIcon: _searchCtrl.text.isNotEmpty
                    ? IconButton(
                        ...
                        onPressed: () {
                          _searchCtrl.clear();
                          context.read<DirectPurchaseController>().clearSearch();
                        },
                      )
                    : null,
                filled: true,
          ...
          Expanded(
            child: Consumer<DirectPurchaseController>(
              builder: (_, ctrl, __) {
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => DirectPurchaseListItem(
                            dp: ctrl.pageItems[i],
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => DirectPurchaseDetailScreen(
  ...
  Widget _buildErrorState(DirectPurchaseController ctrl) {
    return Center(
      child: Padding(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            isSearching ? 'Direct purchase not found' : 'No direct purchases yet',
            style: GoogleFonts.poppins(
              fontSize: 18,
