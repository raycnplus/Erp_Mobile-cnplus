...
import 'package:erp_mobile_cnplus/features/purchase/request_quotation/presentation/widgets/rfq_detail_tabs.dart';
...
class RfqDetailScreen extends StatefulWidget {
  ...
  const RfqDetailScreen({super.key, required this.encryption});
...
class _RfqDetailScreenState extends State<RfqDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final d = ctrl.detail;
    if (d?.isWaitingApproval == true) ctrl.loadSteps(d!.idRfq);
  }
  ...
  Future<void> _handleDelete() async {
    ...
    final ok = await _confirmDialog(
      'Delete Draft',
      'Delete this RFQ draft?',
      confirmLabel: 'Delete',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.remove(d.idRfq);
    if (!mounted) return;
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog('Confirm RFQ', 'Confirm this request for quotation?', confirmLabel: 'Confirm');
    if (ok != true || !mounted) return;

    final f = RfqFormModel.fromDetail(d);
  ...
  Future<void> _handleValidate() async {
    ...
    final ok = await _confirmDialog(
      'Validate RFQ',
      'Validate this RFQ? Status will change to Done.',
  ...
  Future<void> _handleCancel() async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Cancel RFQ', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        content: Column(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text('Cancel RFQ', style: GoogleFonts.poppins()),
          ),
        ],
      ),
    );
    ...
    final success = await ctrl.cancel(d.idRfq, reasonCtrl.text.trim());
    if (!mounted) return;
    _snack(success ? 'Cancelled' : ctrl.formError ?? 'Failed', success: success);
  }

  Future<void> _handleCreatePO() async {
    final ctrl = context.read<RfqController>();
    final d = ctrl.detail;
    if (d == null) return;
    ...
    final ok = await _confirmDialog(
      ...
      'Create a purchase order from this RFQ?',
      confirmLabel: 'Create PO',
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.createPO(d.idRfq);
    if (!mounted) return;
    _snack(success ? ctrl.successMessage! : ctrl.formError ?? 'Failed', success: success);
  }
  ...
  Future<void> _handleApprove() async {
    ...
    final success = await ctrl.approve(d.idRfq);
    if (!mounted) return;
  ...
  Future<void> _handleReject() async {
    ...
    final success = await ctrl.reject(d.idRfq);
    if (!mounted) return;
  ...
  void _handleReturnFromForm(RfqController ctrl) {
    if (!mounted) return;
                  ...
                  if (d.canEdit)
                    IconButton(
                      ...
                      onPressed: () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => RfqFormScreen(encryption: _enc)),
                      ).then((_) {
                        if (!mounted) return;
                        final ne = ctrl.savedEncryption;
                        if (ne != null && ne.isNotEmpty && ne != _enc) {
            ...
            children: [
              Column(
                children: [
                  ...
                  if (d.isDone && d.canCreatePO)
                    _bottomBar([
                      _primaryBtn('Create Purchase Order', Icons.shopping_cart_checkout, _handleCreatePO),
                    ]),
                ],
              ),
              if (ctrl.isSaving)
