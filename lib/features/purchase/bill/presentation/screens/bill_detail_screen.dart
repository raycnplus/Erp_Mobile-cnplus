...
import 'package:erp_mobile_cnplus/features/purchase/bill/presentation/widgets/bill_detail_tabs.dart';
...
class _State extends State<BillDetailScreen> {
  late String _enc;
  ...
  Future<void> _handleDelete() async {
    ...
    final ok = await _confirmDialog(
      'Delete Draft',
      'Delete this bill draft?',
      confirmLabel: 'Delete',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.remove(d.idBill);
    if (!mounted) return;
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog('Confirm Bill', 'Confirm this bill?', confirmLabel: 'Confirm');
    if (ok != true || !mounted) return;

    final f = BillFormModel.fromDetail(d);
  ...
  Future<void> _handlePost() async {
    ...
    final ok = await _confirmDialog(
      'Post Bill',
      'Post this bill? This will create accounting journal entries.',
      confirmLabel: 'Post',
    );
    if (ok != true || !mounted) return;
  ...
  Future<void> _handleCancel() async {
    ...
    final success = await ctrl.cancel(d.idBill, reasonCtrl.text.trim());
    if (!mounted) return;
  ...
  Future<void> _handleCreatePayment() async {
    ...
    final ok = await _confirmDialog('Create Payment', 'Create payment for this bill?', confirmLabel: 'Create');
    if (ok != true || !mounted) return;

    final success = await ctrl.createPayment(d.idBill);
    if (!mounted) return;
    _snack(success ? ctrl.successMessage! : ctrl.formError ?? 'Failed', success: success);
  }
  ...
  Future<void> _handleApprove() async {
    ...
    final success = await ctrl.approve(d.idBill);
    if (!mounted) return;
  ...
  Future<void> _handleReject() async {
    ...
    final success = await ctrl.reject(d.idBill);
    if (!mounted) return;
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) {
                              return BillFormScreen(encryption: _enc);
                            },
                          ),
                        ).then((_) {
                          if (!mounted) return;
