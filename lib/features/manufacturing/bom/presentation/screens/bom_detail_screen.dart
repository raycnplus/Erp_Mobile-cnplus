...
import 'package:erp_mobile_cnplus/features/manufacturing/bom/presentation/widgets/bom_detail_tabs.dart';
...
class BomDetailScreen extends StatefulWidget {
  ...
  const BomDetailScreen({super.key, required this.encryption});
...
class _BomDetailScreenState extends State<BomDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<BomController>().fetchDetail(_enc),
    );
  }
  ...
  Future<void> _handleDelete() async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'This will also delete all components. Are you sure?',
    ...
    if (ok == true && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            success ? 'BOM deleted' : ctrl.formError ?? 'Failed',
          ),
    ...
    return Scaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => BomFormScreen(encryption: _enc),
              ),
            ).then((_) {
              if (!mounted) return;
              final ctrl = context.read<BomController>();
      ...
      body: Consumer<BomController>(
        builder: (_, ctrl, __) {
          ...
          return BomDetailTabs(detail: ctrl.detail!);
        },
      ),
    );
  }
