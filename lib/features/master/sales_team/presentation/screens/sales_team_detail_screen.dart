...
import 'package:erp_mobile_cnplus/features/master/sales_team/presentation/widgets/sales_team_detail_tabs.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'sales_team_form_screen.dart';

class SalesTeamDetailScreen extends StatefulWidget {
  final String encryption;
  const SalesTeamDetailScreen({super.key, required this.encryption});

  @override
  State<SalesTeamDetailScreen> createState() => _SalesTeamDetailScreenState();
}

class _SalesTeamDetailScreenState extends State<SalesTeamDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<SalesTeamController>().fetchTeamDetail(_activeEncryption);
    });
  }
  ...
  Future<void> _handleDelete() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text('Delete Sales Team',
            style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        content: Text('Are you sure you want to delete this sales team?',
    ...
    if (confirmed == true && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(success
            ? 'Sales team deleted successfully'
  ...
  void _navigateToEdit() {
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (_) => SalesTeamFormScreen(encryption: _activeEncryption)),
    ).then((_) {
      if (!mounted) return;
      final controller = context.read<SalesTeamController>();
    ...
    return Scaffold(
      ...
      body: Consumer<SalesTeamController>(
        builder: (context, controller, child) {
          ...
          return SalesTeamDetailTabs(teamDetail: controller.teamDetail!);
        },
      ),
    );
  }
