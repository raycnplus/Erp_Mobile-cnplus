...
import 'package:erp_mobile_cnplus/features/master/product_category/presentation/widgets/product_category_detail_tabs.dart';
...
class ProductCategoryDetailScreen extends StatefulWidget {
  ...
  const ProductCategoryDetailScreen({super.key, required this.encryption});

  @override
  State<ProductCategoryDetailScreen> createState() =>
      _ProductCategoryDetailScreenState();
}

class _ProductCategoryDetailScreenState
    extends State<ProductCategoryDetailScreen> {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success
              ...
              : controller.formError ?? 'Failed to delete category'),
  ...
  void _navigateToEdit() {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            ProductCategoryFormScreen(encryption: _activeEncryption),
      ),
    ).then((_) {
      if (!mounted) return;
      final controller = context.read<ProductCategoryController>();
    ...
    return Scaffold(
      ...
      body: Consumer<ProductCategoryController>(
        builder: (context, controller, child) {
          ...
          return ProductCategoryDetailTabs(
