...
import 'package:erp_mobile_cnplus/features/master/project/presentation/widgets/project_detail_tabs.dart';
...
class ProjectDetailScreen extends StatefulWidget {
  ...
  const ProjectDetailScreen({super.key, required this.encryption});
...
class _ProjectDetailScreenState extends State<ProjectDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<ProjectController>().fetchDetail(_enc),
    );
  }
  ...
  Future<void> _handleDelete() async {
    ...
    if (ok == true && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(success ? 'Project deleted' : ctrl.formError ?? 'Failed'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => ProjectFormScreen(encryption: _enc)),
            ).then((_) {
              if (!mounted) return;
              final ctrl = context.read<ProjectController>();
      ...
      body: Consumer<ProjectController>(builder: (_, ctrl, __) {
        ...
        return ProjectDetailTabs(detail: ctrl.detail!);
      }),
    );
  }
}