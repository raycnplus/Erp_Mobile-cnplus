...
import 'package:erp_mobile_cnplus/features/master/purchase_team/presentation/widgets/purchase_team_list_item.dart';
...
import 'purchase_team_detail_screen.dart';

class PurchaseTeamListScreen extends StatefulWidget {
  const PurchaseTeamListScreen({super.key});

  @override
  State<PurchaseTeamListScreen> createState() => _PurchaseTeamListScreenState();
}

class _PurchaseTeamListScreenState extends State<PurchaseTeamListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final ctrl = context.read<PurchaseTeamController>();
      ctrl.fetchTeamList();
  ...
  void _onSearchChanged(String v) =>
      context.read<PurchaseTeamController>().searchTeams(v);

  void _clearSearch() {
    _searchController.clear();
    context.read<PurchaseTeamController>().clearSearch();
  }

  Future<void> _handleRefresh() =>
      context.read<PurchaseTeamController>().fetchTeamList();

  void _navigateToCreate() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const PurchaseTeamFormScreen()),
    );
  }
  ...
  void _navigateToDetail(String encryption) {
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (_) => PurchaseTeamDetailScreen(encryption: encryption)),
    );
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: Consumer<PurchaseTeamController>(
            ...
            if (isSearching) ...[
              ...
              TextButton.icon(
                ...
                label: Text('Clear search',
                    style:
                        GoogleFonts.poppins(color: colorPrimary, fontSize: 14)),
              ),
            ] else ...[
              const SizedBox(height: 8),
              Text('Tap + button to add your first team',
                  style: GoogleFonts.poppins(
                      fontSize: 14, color: colorGreyDark)),
            ],
          ],
        ),
      ),
