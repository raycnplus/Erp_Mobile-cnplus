...
import 'package:erp_mobile_cnplus/features/master/brand/presentation/widgets/brand_detail_tabs.dart';
...
class BrandDetailScreen extends StatefulWidget {
  ...
  const BrandDetailScreen({super.key, required this.encryption});
...
class _BrandDetailScreenState extends State<BrandDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<BrandController>().fetchBrandDetail(_activeEncryption);
    });
  }
  ...
  Future<void> _handleDelete() async {
    ...
    if (confirmed == true && mounted) {
      ...
      final success = await controller.removeBrand(_activeEncryption);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success
              ...
              : controller.formError ?? 'Failed to delete brand'),
  ...
  void _navigateToEdit() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => BrandFormScreen(encryption: _activeEncryption),
      ),
    ).then((_) {
      if (!mounted) return;
      final controller = context.read<BrandController>();
    ...
    return Scaffold(
      ...
      body: Consumer<BrandController>(
        builder: (context, controller, child) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () =>
                        controller.fetchBrandDetail(_activeEncryption),
                    style: ElevatedButton.styleFrom(
          ...
          return BrandDetailTabs(brandDetail: controller.brandDetail!);
        },
      ),
    );
  }
