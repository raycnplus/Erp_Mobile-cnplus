...
import 'package:erp_mobile_cnplus/features/master/location/presentation/widgets/location_list_item.dart';
...
class _LocationListScreenState extends State<LocationListScreen> {
  ...
  void _onSearchChanged(String value) =>
      context.read<LocationController>().searchLocations(value);

  void _clearSearch() {
    _searchController.clear();
    context.read<LocationController>().clearSearch();
  }

  Future<void> _handleRefresh() async =>
      context.read<LocationController>().fetchLocationList();
  ...
  void _navigateToDetail(String encryption) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => LocationDetailScreen(encryption: encryption),
      ),
    );
  }
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search name, code or warehouse...',
          ...
          Expanded(
            child: Consumer<LocationController>(
              builder: (context, ctrl, child) {
                ...
                if (ctrl.listError != null && ctrl.locationList.isEmpty) {
                  return _buildErrorState(ctrl.listError!, _handleRefresh);
                }

                if (ctrl.locationList.isEmpty) {
                  return Column(
                    children: [
                      Expanded(
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemCount: ctrl.locationList.length,
                          itemBuilder: (context, index) {
                            final loc = ctrl.locationList[index];
                            return LocationListItem(
                              location: loc,
                              onTap: () => _navigateToDetail(loc.encryption),
                            );
                          },
                        ),
                      ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              isSearching ? 'Location not found' : 'No locations yet',
              style: GoogleFonts.poppins(
                  fontSize: 18,
            ...
            if (isSearching) ...[
              ...
              TextButton.icon(
                onPressed: _clearSearch,
                icon:
                    const Icon(Icons.close, size: 16, color: colorPrimary),
              ...
              const SizedBox(height: 8),
              Text(
                'Tap + button to add your first location',
                style: GoogleFonts.poppins(fontSize: 14, color: colorGreyDark),
                textAlign: TextAlign.center,
              ),
            ],
