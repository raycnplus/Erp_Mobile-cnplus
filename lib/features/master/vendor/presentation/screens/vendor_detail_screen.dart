...
import 'package:erp_mobile_cnplus/features/master/vendor/presentation/widgets/vendor_detail_tabs.dart';
...
class VendorDetailScreen extends StatefulWidget {
  ...
  const VendorDetailScreen({super.key, required this.encryption});
...
class _VendorDetailScreenState extends State<VendorDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<VendorController>().fetchVendorDetail(_activeEncryption);
    });
  }
  ...
  Future<void> _handleDelete() async {
    ...
    if (confirmed == true && mounted) {
      ...
      final success = await controller.removeVendor(_activeEncryption);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success
              ...
              : controller.formError ?? 'Failed to delete vendor'),
  ...
  void _navigateToEdit() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => VendorFormScreen(encryption: _activeEncryption),
      ),
    ).then((_) {
      if (!mounted) return;
      final controller = context.read<VendorController>();
    ...
    return Scaffold(
      ...
      body: Consumer<VendorController>(
        builder: (context, controller, child) {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () =>
                          controller.fetchVendorDetail(_activeEncryption),
                      style: ElevatedButton.styleFrom(
          ...
          if (controller.vendorDetail == null) {
            return Center(
                child: Text("No data available",
                    style: GoogleFonts.poppins(color: colorGrey)));
          }

          return VendorDetailTabs(vendorDetail: controller.vendorDetail!);
        },
      ),
    );
  }
