...
import 'package:erp_mobile_cnplus/features/pos/store/presentation/widgets/store_detail_tabs.dart';
...
class StoreDetailScreen extends StatefulWidget {
  ...
  const StoreDetailScreen({super.key, required this.encryption});
...
class _StoreDetailScreenState extends State<StoreDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<StoreController>().fetchDetail(_enc),
    );
  }
  ...
  Future<void> _handleDelete() async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('This store will be deleted. Are you sure?',
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(success ? 'Store deleted' : ctrl.formError ?? 'Failed'),
    ...
    return Scaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (_) => StoreFormScreen(encryption: _enc)),
            ).then((_) {
              if (!mounted) return;
              final ctrl = context.read<StoreController>();
      ...
      body: Consumer<StoreController>(
        builder: (_, ctrl, __) {
          ...
          return StoreDetailTabs(detail: ctrl.detail!);
        },
      ),
    );
  }
