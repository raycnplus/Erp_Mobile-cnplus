...
import '../widget/show_widget_vendor_purchase.dart'; // DIUBAH: Menggunakan widget purchase
// DIUBAH: Path lengkap ke update screen (diambil dari file lama kamu)
...
class _VendorShowScreenState extends State<VendorShowScreen> {
  ...
  bool hasChanged = false; // Untuk melacak apakah ada perubahan
  ...
  Future<bool> _deleteVendor() async {
    ...
    // DIUBAH: Endpoint ke purchase vendor
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvoked: (bool didPop) {
        if (didPop) return;
        Navigator.pop(context, hasChanged); // Kirim status 'hasChanged'
      },
      child: Scaffold(
        ...
        appBar: AppBar(
          leading: IconButton(
            ...
            // Kirim status 'hasChanged' saat tombol back ditekan
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () async {
            ...
            // JIKA UPDATE SUKSES (true), kembali ke index
            if (resultFromUpdate == true && mounted) {
              // 'true' akan ditangkap oleh halaman index untuk refresh
              Navigator.pop(context, true);
            } else {
              // Jika update tidak terjadi atau dibatalkan, muat ulang data detail
              // (refreshTrigger) dan tandai 'hasChanged' jika ada perubahan.
              setState(() {
                if (resultFromUpdate == true) {
                  hasChanged = true;
                }
