...
import '../../update/screen/purchase_team_update_screen.dart';

class PurchaseTeamShowScreen extends StatefulWidget {
  final int teamId;

  const PurchaseTeamShowScreen({super.key, required this.teamId});

  @override
  State<PurchaseTeamShowScreen> createState() => _PurchaseTeamShowScreenState();
}

class _PurchaseTeamShowScreenState extends State<PurchaseTeamShowScreen> {
  ...
  Future<void> _navigateToUpdate() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PurchaseTeamUpdateScreen(id: widget.teamId),
      ),
    );
  ...
  Future<void> _deleteTeam() async {
    ...
    try {
      final response = await http.delete(
        Uri.parse("${ApiBase.baseUrl}/purchase/purchase-team/${widget.teamId}"),
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text("Detail Tim Pembelian", style: GoogleFonts.poppins(color: Colors.black87, fontWeight: FontWeight.w600)),
          centerTitle: true,
          actions: [
            IconButton(
              tooltip: 'Hapus Tim',
        ...
        body: PurchaseTeamShowWidget(key: _childKey, teamId: widget.teamId),
        floatingActionButton: TweenAnimationBuilder<double>(
          ...
          child: FloatingActionButton.extended(
            ...
            backgroundColor: Colors.blueAccent, // Warna FAB disesuaikan
          ),
        ),
      ),
    );
