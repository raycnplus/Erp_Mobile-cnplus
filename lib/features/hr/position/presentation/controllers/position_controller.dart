...
class PositionController extends ChangeNotifier {
  final GetPositionList getPositionList;
  final GetPositionDetail getPositionDetail;
  final CreatePosition createPosition;
  final UpdatePosition updatePosition;
  final DeletePosition deletePosition;

  PositionController({
    required this.getPositionList,
    required this.getPositionDetail,
    required this.createPosition,
    required this.updatePosition,
    required this.deletePosition,
  });

  List<PositionModel> _all = [];
  List<PositionModel> _filtered = [];
  ...
  List<PositionModel> get pageItems {
  ...
  PositionDetailModel? get detail => _detail;
  ...
  Future<void> fetchList() async {
    ...
    notifyListeners();
    try {
      final all = <PositionModel>[];
      int page = 1;
      while (true) {
        final result = await getPositionList(page: page, perPage: 100);
        final meta = result['meta'] as PositionPaginationMeta;
        all.addAll(result['items'] as List<PositionModel>);
  ...
  void _applyFilter() {
    ...
    _filtered = _all.where((item) {
      return item.positionName.toLowerCase().contains(query) ||
          (item.positionDescription?.toLowerCase().contains(query) ?? false);
    }).toList();
  }
  ...
  Future<void> fetchDetail(String enc) async {
    ...
    notifyListeners();
    try {
      _detail = await getPositionDetail(enc);
    } catch (e) {
      _detailError = _msg(e);
      _detail = null;
    } finally {
  ...
  Future<bool> save(PositionFormModel form) async {
    _isSaving = true;
    _formError = null;
    ...
    try {
      ...
      _successMessage = 'Position created successfully';
      await fetchList();
      return true;
    } catch (e) {
      _formError = _msg(e);
  ...
  Future<bool> edit(String enc, PositionFormModel form) async {
    _isSaving = true;
    _formError = null;
    _successMessage = null;
    ...
    notifyListeners();
    try {
      final newEnc = await updatePosition(enc, form);
      _updatedEncryption = newEnc;
      _successMessage = 'Position updated successfully';
      await fetchList();
  ...
  Future<bool> remove(String enc) async {
    ...
    notifyListeners();
    try {
      await deletePosition(enc);
      _successMessage = 'Position deleted successfully';
      await fetchList();
