...
import 'package:erp_mobile_cnplus/features/hr/overtime_request/presentation/widgets/overtime_request_list_item.dart';
...
class OvertimeRequestListScreen extends StatefulWidget {
  const OvertimeRequestListScreen({super.key});

  @override
  State<OvertimeRequestListScreen> createState() =>
      _OvertimeRequestListScreenState();
}

class _OvertimeRequestListScreenState
    extends State<OvertimeRequestListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<OvertimeRequestController>().fetchList(),
    );
  }
  ...
  void _goToForm() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const OvertimeRequestFormScreen()),
    ).then((_) {
      context.read<OvertimeRequestController>().fetchList(status: _selectedStatus);
    });
  }
        ...
        children: [
          Container(
            ...
            child: TextField(
              ...
              onChanged: (v) =>
                  context.read<OvertimeRequestController>().search(v),
              decoration: InputDecoration(
                hintText: 'Search employee or overtime type...',
                ...
                suffixIcon: _searchCtrl.text.isNotEmpty
                    ? IconButton(
                        ...
                        onPressed: () {
                          _searchCtrl.clear();
                          context
                              .read<OvertimeRequestController>()
                              .clearSearch();
                        },
                      )
                    : null,
          ...
          Expanded(
            child: Consumer<OvertimeRequestController>(
              builder: (_, ctrl, __) {
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => OvertimeRequestListItem(
                            req: ctrl.pageItems[i],
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => OvertimeRequestDetailScreen(
                                  encryption: ctrl.pageItems[i].encryption,
                                ),
                              ),
                            ).then(
                              (_) => ctrl.fetchList(status: _selectedStatus),
                            ),
                          ),
                        ),
                      ),
  ...
  Widget _error(OvertimeRequestController ctrl) {
    return Center(
      child: Padding(
        ...
        children: [
          ...
          if (isSearching) ...[
            ...
            TextButton.icon(
              onPressed: () {
                _searchCtrl.clear();
                context.read<OvertimeRequestController>().clearSearch();
              },
