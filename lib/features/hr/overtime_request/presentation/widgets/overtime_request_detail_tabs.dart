...
class OvertimeRequestDetailTabs extends StatefulWidget {
  final OvertimeRequestDetailModel detail;
  final VoidCallback? onApprove;
  final VoidCallback? onReject;
  final bool canApprove;

  const OvertimeRequestDetailTabs({
    super.key,
  ...
  @override
  State<OvertimeRequestDetailTabs> createState() =>
      _OvertimeRequestDetailTabsState();
}

class _OvertimeRequestDetailTabsState
    extends State<OvertimeRequestDetailTabs>
  ...
  static const _statusColors = <String, Color>{
    ...
    'Waiting Approval': Color(0xFFE65100),
                  ...
                  children: [
                    _infoCard(
                      ...
                      [
                        ...
                        _row('Overtime Type', _safe(d.overtimeTypeName)),
                        _row('Request Date', _fmt(d.requestDate)),
                        _row(
                          'Start Time',
                          _safe(
                            d.startDatetime?.length == 16
                                ? d.startDatetime!.substring(11)
                                : d.startDatetime,
                          ),
                        ),
                        _row(
                          'End Time',
                          _safe(
                            d.endDatetime?.length == 16
                                ? d.endDatetime!.substring(11)
                                : d.endDatetime,
                          ),
                        ),
                        _row(
                          'Requested Hours',
                          '${d.requestedHours.toStringAsFixed(1)} hours',
                        ),
                        if (d.approvedHours > 0)
                          _row(
                            'Approved Hours',
                            '${d.approvedHours.toStringAsFixed(1)} hours',
                          ),
                        if (d.reason?.isNotEmpty == true)
