...
import 'package:erp_mobile_cnplus/features/hr/leave_type/presentation/widgets/leave_type_detail_tabs.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'leave_type_form_screen.dart';

class LeaveTypeDetailScreen extends StatefulWidget {
  final String encryption;

  const LeaveTypeDetailScreen({super.key, required this.encryption});

  @override
  State<LeaveTypeDetailScreen> createState() => _LeaveTypeDetailScreenState();
}

class _LeaveTypeDetailScreenState extends State<LeaveTypeDetailScreen> {
  late String _enc;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<LeaveTypeController>().fetchDetail(_enc),
    );
  }
  ...
  Future<void> _handleDelete() async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: Text('Delete Leave Type', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(success ? 'Leave type deleted' : ctrl.formError ?? 'Failed to delete'),
    ...
    return Scaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => LeaveTypeFormScreen(encryption: _enc)),
            ).then((_) {
              if (!mounted) return;
              final ctrl = context.read<LeaveTypeController>();
      ...
      body: Consumer<LeaveTypeController>(
        builder: (_, ctrl, __) {
          ...
          return LeaveTypeDetailTabs(detail: ctrl.detail!);
        },
      ),
    );
  }
