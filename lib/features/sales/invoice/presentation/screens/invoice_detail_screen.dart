...
import 'package:erp_mobile_cnplus/features/sales/invoice/presentation/widgets/invoice_detail_tabs.dart';
...
class InvoiceDetailScreen extends StatefulWidget {
  ...
  const InvoiceDetailScreen({super.key, required this.encryption});
...
class _InvoiceDetailScreenState extends State<InvoiceDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final d = ctrl.detail;
    if (d?.isWaitingApproval == true) ctrl.loadSteps(d!.idInvoice);
  }
  ...
  Future<void> _handleDelete() async {
    ...
    final ok = await _confirmDialog(
      'Delete Draft',
      'Delete this invoice draft?',
      confirmLabel: 'Delete',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.remove(d.idInvoice);
    if (!mounted) return;
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog('Confirm Invoice', 'Confirm this invoice?', confirmLabel: 'Confirm');
    if (ok != true || !mounted) return;

    final f = InvoiceFormModel.fromDetail(d);
  ...
  Future<void> _handleValidate() async {
    ...
    final autoCreateJournal = ctrl.formOptions?.autoCreateJournal ?? false;
    DateTime journalDate = DateTime.now();

    if (autoCreateJournal) {
      ...
      final ok = await _confirmDialog(
        'Validate Invoice',
        'Validate this invoice? Status will change to Posted.',
        confirmLabel: 'Validate',
      );
      if (ok != true || !mounted) return;
    }

    final f = InvoiceFormModel.fromDetail(d)..journalTransactionDate = journalDate;
  ...
  Future<void> _handleCancel() async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Cancel Invoice', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        content: Column(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text('Cancel Invoice', style: GoogleFonts.poppins()),
          ),
        ],
      ),
    );
    ...
    final success = await ctrl.cancel(d.idInvoice, reasonCtrl.text.trim());
    if (!mounted) return;
  ...
  Future<void> _handleCreatePayment() async {
    ...
    final success = await ctrl.createPayment(d.idInvoice);
    if (!mounted) return;
    _snack(success ? ctrl.successMessage! : ctrl.formError ?? 'Failed', success: success);
  }
  ...
  Future<void> _handleApprove() async {
    ...
    final success = await ctrl.approve(d.idInvoice);
    if (!mounted) return;
  ...
  Future<void> _handleReject() async {
    ...
    final success = await ctrl.reject(d.idInvoice);
    if (!mounted) return;
  ...
  Future<void> _handleViewSteps() async {
    ...
    await ctrl.loadSteps(d.idInvoice);
    if (!mounted) return;
                  ...
                  if (d.canEdit)
                    IconButton(
                      ...
                      onPressed: () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => InvoiceFormScreen(encryption: _enc)),
                      ).then((_) {
                        if (!mounted) return;
          ...
          return Stack(
            children: [
              Column(
                children: [
                  Expanded(
                    child: InvoiceDetailTabs(
                      detail: d,
                      onCreatePayment: d.canCreatePayment && !d.hasPayment ? _handleCreatePayment : null,
                    ),
                  ),
                  if (d.isDraft)
                    _bottomBar([
                  ...
                  if (d.isPosted && d.canCreatePayment && !d.hasPayment)
                    _bottomBar([
