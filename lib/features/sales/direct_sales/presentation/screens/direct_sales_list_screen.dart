...
import 'package:erp_mobile_cnplus/features/sales/direct_sales/presentation/widgets/direct_sales_list_item.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'package:erp_mobile_cnplus/shared/widgets/pagination_bottom_bar.dart';
import 'direct_sales_detail_screen.dart';
import 'direct_sales_form_screen.dart';

class DirectSalesListScreen extends StatefulWidget {
  const DirectSalesListScreen({super.key});

  @override
  State<DirectSalesListScreen> createState() => _State();
}

class _State extends State<DirectSalesListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => context.read<DirectSalesController>().fetchList(),
    );
  }
  ...
  void _goToForm() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const DirectSalesFormScreen()),
    ).then((_) {
      context.read<DirectSalesController>().fetchList(status: _selectedStatus);
    });
  }
        ...
        children: [
          Container(
            ...
            child: TextField(
              ...
              onChanged: (v) =>
                  context.read<DirectSalesController>().search(v),
          ...
          Expanded(
            child: Consumer<DirectSalesController>(
              builder: (_, ctrl, __) {
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => DirectSalesListItem(
                            ds: ctrl.pageItems[i],
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => DirectSalesDetailScreen(
                                  encryption:
  ...
  Widget _buildErrorState(DirectSalesController ctrl) {
    return Center(
      child: Padding(
        ...
        children: [
          ...
          if (isSearching) ...[
            ...
            TextButton.icon(
              onPressed: () {
                _searchCtrl.clear();
                context.read<DirectSalesController>().clearSearch();
              },
