...
class DirectSalesDetailTabs extends StatefulWidget {
  final DirectSalesDetailModel detail;
  final VoidCallback? onCreateInvoiceFromTerm;

  const DirectSalesDetailTabs({
    super.key,
  ...
  @override
  State<DirectSalesDetailTabs> createState() => _State();
}

class _State extends State<DirectSalesDetailTabs>
  ...
  Widget _buildProductsTab(DirectSalesDetailModel d) {
    return Column(
      children: [
        Expanded(
  ...
  Widget _buildInfoTab(DirectSalesDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          _infoCard('Direct Sales Info', Icons.point_of_sale_outlined, [
          ...
          if (d.hasInvoice || d.hasDeliveryNote) ...[
            const SizedBox(height: 12),
  ...
  Widget _buildPaymentTab(DirectSalesDetailModel d) {
    if (!d.isMultiPayment) {
      return SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _scheduleCard(
    DirectSalesPaymentSchedule s,
    DirectSalesDetailModel d,
  ) {
    Color statusColor;
    switch (s.status) {
      case 'Paid':
  ...
  Widget _invoiceCard(DirectSalesInvoice inv) {
    return Card(
      elevation: 1,
      margin: const EdgeInsets.only(bottom: 8),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
  ...
  Widget _buildOthersTab(DirectSalesDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          ...
          if (d.cancelledByName?.isNotEmpty == true ||
              d.cancelledDate?.isNotEmpty == true) ...[
            const SizedBox(height: 12),
