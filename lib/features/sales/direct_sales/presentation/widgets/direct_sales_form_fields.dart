...
class DirectSalesFormFields extends StatefulWidget {
  final DirectSalesFormModel formData;
  final DirectSalesFormOptions? formOptions;
  final VoidCallback onSave;
  final bool isSaving;

  const DirectSalesFormFields({
    super.key,
  ...
  @override
  State<DirectSalesFormFields> createState() => _State();
}

class _State extends State<DirectSalesFormFields> with TickerProviderStateMixin {
  ...
  DirectSalesFormOptions? get _opts => widget.formOptions;
  ...
  bool _validateAndSync() {
    ...
    if (widget.formData.deliveryAddress.isEmpty) { _snack('Please enter delivery address');    return false; }
  ...
  Future<void> _addProduct() async {
    ...
    final products = await ctrl.fetchProductsByLocation(widget.formData.sourceLocation!);
    if (!mounted) return;
    if (products.isEmpty) {
    ...
    DSProductOption? selected;
    DSUomOption? selectedUom;
        ...
        builder: (ctx, setSt) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                CustomSearchableDropdown<DSProductOption>(
                  value: selected,
                  items: products,
                ...
                if (selected != null)
                  CustomSearchableDropdown<DSUomOption>(
                    value: selectedUom,
                    items: _opts?.uoms ?? [],
                    itemLabel: (u) => u.name,
    ...
    if (result != null && mounted) {
      final prod = result['product'] as DSProductOption;
      final uom  = result['uom']    as DSUomOption;
      double unitPrice = 0;
      if (widget.formData.idPriceList != null) {
        unitPrice = await ctrl.fetchPriceFromList(prod.idProduct, widget.formData.idPriceList!) ?? 0;
      }
      setState(() {
        widget.formData.items.add(DirectSalesFormItem(
  ...
  List<DSLocationOption> _getFilteredLocations() {
    if (_opts == null || widget.formData.sourceWarehouse == null) return [];
    final filtered = _opts!.locations.where((l) => l.warehouseId == widget.formData.sourceWarehouse).toList();
  ...
  Widget _buildHeaderTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          CustomSearchableDropdown<DSCustomerOption>(
          ...
          CustomSearchableDropdown<DSWarehouseOption>(
          ...
          CustomSearchableDropdown<DSLocationOption>(
          ...
          CustomSearchableDropdown<DSSalesPersonOption>(
          ...
          CustomSearchableDropdown<DSPaymentTermOption>(
          ...
          CustomSearchableDropdown<DSPriceListOption>(
          ...
          CustomFormInput(
            ...
            label: 'Note',
            hintText: 'Write note for this direct sales (optional)',
            maxLines: 3,
          ),
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      ...
                      style: GoogleFonts.poppins(
                        fontSize: 11,
                        color: item.onHand < item.demandQty && item.demandQty > 0
                            ? colorError
                            : colorGrey,
                      ),
                    ),
  ...
  void _addSchedule() {
    setState(() {
      widget.formData.schedules.add(DirectSalesScheduleItem(
  ...
  Widget _buildBottomBar() => Container(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(
              widget.formData.isEditMode ? 'Update Direct Sales' : 'Save Direct Sales',
