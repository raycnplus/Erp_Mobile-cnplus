...
class ServiceDirectSalesDetailTabs extends StatefulWidget {
  final ServiceDirectSalesDetailModel detail;
  final void Function(ServiceDirectSalesPaymentSchedule schedule)?
      onCreateInvoiceFromTerm;

  const ServiceDirectSalesDetailTabs({
    super.key,
  ...
  @override
  State<ServiceDirectSalesDetailTabs> createState() => _State();
}

class _State extends State<ServiceDirectSalesDetailTabs>
  ...
  Widget _buildServicesTab(ServiceDirectSalesDetailModel d) {
    return Column(
      children: [
        Expanded(
  ...
  Widget _buildInfoTab(ServiceDirectSalesDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          _infoCard(
            'Service Direct Sales Info',
          ...
          if (d.hasServiceInvoice) ...[
            const SizedBox(height: 12),
            _infoCard('Linked Invoice', Icons.receipt_outlined, [
              _row('Service Invoice', 'Linked'),
            ]),
          ],
          const SizedBox(height: 12),
        ],
  ...
  Widget _buildPaymentTab(ServiceDirectSalesDetailModel d) {
    if (!d.isMultiPayment) {
      return SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      d.isDone
                          ? 'Payment Type: Full — Service Invoice auto-created on validate.'
                          : 'Payment Type: Full — Service Invoice will be auto-created on validate.',
                      style: GoogleFonts.poppins(
                        fontSize: 12,
                        color: colorPrimary,
                      ),
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) => _buildScheduleCard(d.paymentSchedules[i], d),
    );
  }

  Widget _buildScheduleCard(
    ServiceDirectSalesPaymentSchedule s,
    ServiceDirectSalesDetailModel d,
  ) {
    Color statusColor;
    switch (s.status) {
      case 'Paid':
            ...
            if (d.isDone &&
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: () => widget.onCreateInvoiceFromTerm!(s),
  ...
  Widget _buildOthersTab(ServiceDirectSalesDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          ...
          if (d.cancelledByName?.isNotEmpty == true || d.cancelledReason?.isNotEmpty == true) ... [
            const SizedBox(height: 12),
