...
class ServiceDirectSalesFormFields extends StatefulWidget {
  final ServiceDirectSalesFormModel formData;
  final ServiceDirectSalesFormOptions? formOptions;
  final VoidCallback onSave;
  final bool isSaving;

  const ServiceDirectSalesFormFields({
    super.key,
  ...
  @override
  State<ServiceDirectSalesFormFields> createState() => _State();
}

class _State extends State<ServiceDirectSalesFormFields> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _tab           = TabController(length: widget.formData.isMultiPayment ? 3 : 2, vsync: this);
  ...
  ServiceDirectSalesFormOptions? get _opts => widget.formOptions;
  ...
  bool _validateAndSync() {
    ...
    if (widget.formData.salesPerson == null) { _snack('Please select sales person');      return false; }
    if (widget.formData.items.isEmpty)       { _snack('Please add at least one service'); return false; }

    for (int i = 0; i < widget.formData.items.length; i++) {
      final item = widget.formData.items[i];
      if (item.idService == null) { _snack('Row ${i + 1}: Please select service'); return false; }
      if (item.qty <= 0)          { _snack('Row ${i + 1}: Qty must be > 0');       return false; }
    }
    ...
    if (widget.formData.isMultiPayment) {
      ...
      // Backend: total schedule amount = untaxedAmount (sama dengan SSO)
      final totalAmt = widget.formData.totalScheduleAmount;
      final untaxed  = widget.formData.untaxedAmount;
      if ((totalAmt - untaxed).abs() > 0.5) {
        _snack(
          'Total schedule amount (Rp ${totalAmt.toStringAsFixed(0)}) '
          'must equal Untaxed Amount (Rp ${untaxed.toStringAsFixed(0)})',
        );
        return false;
      }
    }
  ...
  Future<void> _addService() async {
    ...
    SDSServiceOption? selected;
    final result = await showDialog<SDSServiceOption>(
      context: context,
        ...
        builder: (ctx, setSt) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                CustomSearchableDropdown<SDSServiceOption>(
                  value: selected,
                  items: services,
    ...
    if (result != null && mounted) {
      ...
      if (widget.formData.idPriceList != null) {
        final priced = await ctrl.fetchPriceFromList(result.idService, widget.formData.idPriceList!);
        if (priced != null && priced > 0) unitPrice = priced;
      }
      setState(() {
        widget.formData.items.add(ServiceDirectSalesFormItem(
  ...
  bool _scheduleOk() {
    final pctOk = (widget.formData.totalSchedulePercentage - 100).abs() <= 0.01;
    final amtOk = (widget.formData.totalScheduleAmount - widget.formData.untaxedAmount).abs() <= 0.5;
    return pctOk && amtOk;
  }
  ...
  Widget _buildHeaderTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          CustomSearchableDropdown<SDSCustomerOption>(
          ...
          CustomSearchableDropdown<SDSSalesPersonOption>(
          ...
          _datePicker(
            ...
            hint: 'Select validity date (optional)',
          ...
          CustomSearchableDropdown<SDSPaymentTermOption>(
          ...
          CustomSearchableDropdown<SDSPriceListOption>(
          ...
          child: widget.formData.items.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      const Icon(Icons.miscellaneous_services_outlined, size: 64, color: colorGrey),
  ...
  Widget _buildSchedulesTab() {
    ...
    final amtOk    = (totalAmt - untaxed).abs() <= 0.5;
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'Untaxed Amount: Rp ${_fmtNum(untaxed)}',
                      style: GoogleFonts.poppins(fontSize: 12, color: colorTextSubtle),
                    ),
                    Text(
                      'Schedule amounts must total this (PPN handled separately)',
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: GoogleFonts.poppins(
                            fontSize: 12,
                            fontWeight: FontWeight.w600,
                            color: amtOk ? Colors.green : Colors.orange,
                          ),
                        ),
                      ],
                    ),
              ...
              OutlinedButton.icon(
                onPressed: () => setState(() => widget.formData.schedules.add(
                ...
                Expanded(
                  child: _numField(
                    ...
                    onChanged: (v) => setState(() {
                      ...
                      s.amount     = untaxed * (s.percentage / 100);
                      _schedCtrls['sched_${i}_amount']?.text = s.amount.toStringAsFixed(2);
                    }),
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: _numField(
                    ...
                    onChanged: (v) => setState(() {
                      ...
                      _schedCtrls['sched_${i}_percentage']?.text = s.percentage.toStringAsFixed(2);
                    }),
                  ),
                ),
              ],
  ...
  Widget _buildBottomBar() => Container(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(
              widget.formData.isEditMode ? 'Update Service Direct Sales' : 'Save Service Direct Sales',
