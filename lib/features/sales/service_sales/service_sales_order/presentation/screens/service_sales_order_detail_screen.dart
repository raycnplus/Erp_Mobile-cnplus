...
import 'package:erp_mobile_cnplus/features/sales/service_sales/service_sales_order/presentation/widgets/service_sales_order_detail_tabs.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'service_sales_order_form_screen.dart';

class ServiceSalesOrderDetailScreen extends StatefulWidget {
  final String encryption;

  const ServiceSalesOrderDetailScreen({super.key, required this.encryption});

  @override
  State<ServiceSalesOrderDetailScreen> createState() => _State();
}

class _State extends State<ServiceSalesOrderDetailScreen> {
  late String _enc;
  ...
  Future<void> _load() async {
    ...
    final d = ctrl.detail;
    if (d?.isWaitingApproval == true) ctrl.loadSteps(d!.idServiceSalesOrder);
  }
  ...
  Future<void> _handleDelete() async {
    ...
    final ok = await _confirmDialog(
      'Delete Draft',
      'Delete this service sales order draft?',
      confirmLabel: 'Delete',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.remove(d.idServiceSalesOrder);
    if (!mounted) return;
    _snack(
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog(
      'Confirm Service Sales Order',
      'Confirm this service sales order?',
      confirmLabel: 'Confirm',
    );
    if (ok != true || !mounted) return;

    final f = ServiceSalesOrderFormModel.fromDetail(d);
  ...
  Future<void> _handleValidate() async {
    ...
    final ok = await _confirmDialog(
      'Validate Service Sales Order',
      'Validate this SSO? Status will change to Done.',
  ...
  Future<void> _handleCancel() async {
    ...
    final success = await ctrl.cancel(
      d.idServiceSalesOrder,
  ...
  Future<void> _handleClose() async {
    ...
    final ok = await _confirmDialog(
      'Close Service Sales Order',
      'Close this service sales order?',
      confirmLabel: 'Close',
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.close(d.idServiceSalesOrder);
    if (!mounted) return;
    _snack(
      success ? 'Closed' : ctrl.formError ?? 'Failed',
      success: success,
    );
  }
  ...
  Future<void> _handleCreateInvoice() async {
    ...
    final ok = await _confirmDialog(
      'Create Invoice',
      'Create invoice from this service sales order?',
      confirmLabel: 'Create',
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.createInvoice(d.idServiceSalesOrder);
    if (!mounted) return;
    _snack(
      success ? ctrl.successMessage! : ctrl.formError ?? 'Failed',
      success: success,
  ...
  Future<void> _handleCreateInvoiceFromTerm() async {
    ...
    ServiceSalesOrderPaymentSchedule? selected = pendingSchedules.first;
        ...
        builder: (ctx, setSt) => AlertDialog(
          ...
          content: Column(
            ...
            children: pendingSchedules.map((s) {
              return RadioListTile<ServiceSalesOrderPaymentSchedule>(
                value: s,
                groupValue: selected,
  ...
  Future<void> _handleApprove() async {
    ...
    final ok = await _confirmDialog(
      'Approve',
      'Approve this service sales order?',
      confirmLabel: 'Approve',
      confirmColor: const Color(0xFF2E7D32),
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.approve(d.idServiceSalesOrder);
    if (!mounted) return;
    _snack(
  ...
  Future<void> _handleReject() async {
    ...
    final ok = await _confirmDialog(
      'Reject',
      'Reject this service sales order?',
      confirmLabel: 'Reject',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.reject(d.idServiceSalesOrder);
    if (!mounted) return;
    _snack(
  ...
  Future<void> _handleViewSteps() async {
    ...
    await ctrl.loadSteps(d.idServiceSalesOrder);
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 1,
        actions: [
          Consumer<ServiceSalesOrderController>(
            builder: (_, ctrl, __) {
              final d = ctrl.detail;
                  ...
                  if (d.canEdit)
                    IconButton(
                      ...
                      onPressed: () => Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => ServiceSalesOrderFormScreen(
                            encryption: _enc,
                          ),
                        ),
                      ).then((_) {
          ...
          return Stack(
            children: [
              Column(
                children: [
                  Expanded(
                    child: ServiceSalesOrderDetailTabs(
                      detail: d,
