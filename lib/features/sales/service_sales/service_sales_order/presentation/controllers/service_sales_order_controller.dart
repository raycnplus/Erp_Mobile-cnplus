...
class ServiceSalesOrderController extends ChangeNotifier {
  final GetSSOList getList;
  final GetSSODetail getDetail;
  final GetSSOFormOptions getFormOptions;
  final SaveServiceSalesOrder saveSSO;
  final CancelServiceSalesOrder cancelSSO;
  final CloseServiceSalesOrder closeSSO;
  final DeleteServiceSalesOrder deleteSSO;
  final CreateInvoiceFromSSO createInvoiceUC;
  final CreateInvoiceFromSSOTerm createInvoiceFromTermUC;
  final GetSSOPriceFromList getPriceFromList;
  final ApproveSSO approveSSO;
  final RejectSSO rejectSSO;
  final GetSSOSteps getSteps;

  ServiceSalesOrderController({
    required this.getList,
    required this.getDetail,
    required this.getFormOptions,
    required this.saveSSO,
    required this.cancelSSO,
    required this.closeSSO,
    required this.deleteSSO,
    required this.createInvoiceUC,
    required this.createInvoiceFromTermUC,
    required this.getPriceFromList,
    required this.approveSSO,
    required this.rejectSSO,
    required this.getSteps,
  });

  List<ServiceSalesOrderModel> _all = [];
  List<ServiceSalesOrderModel> _filtered = [];
  ...
  ServiceSalesOrderDetailModel? _detail;
  bool _isLoadingDetail = false;
  String? _detailError;

  ServiceSalesOrderFormOptions? _formOptions;
  ...
  List<ServiceSalesOrderModel> get pageItems {
  ...
  ServiceSalesOrderDetailModel? get detail => _detail;
  bool get isLoadingDetail => _isLoadingDetail;
  String? get detailError => _detailError;
  ServiceSalesOrderFormOptions? get formOptions => _formOptions;
  ...
  Future<void> fetchList({String? status}) async {
    ...
    notifyListeners();
    try {
      final all = <ServiceSalesOrderModel>[];
      int p = 1;
      while (true) {
        ...
        final meta = r['meta'] as ServiceSalesOrderPaginationMeta;
        all.addAll(r['items'] as List<ServiceSalesOrderModel>);
  ...
  Future<bool> save(
    ...
    try {
      ...
      _savedEncryption = await saveSSO(f, status, defaultTaxRate: taxRate);
      _successMessage = switch (status) {
        'confirm' => 'Service Sales Order confirmed',
        'validate' => 'Service Sales Order validated',
        _ => f.isEditMode
            ? 'Service Sales Order updated'
            : 'Service Sales Order created',
      };
  ...
  Future<bool> cancel(int id, String reason) async {
    ...
    notifyListeners();
    try {
      await cancelSSO(id, reason);
      _successMessage = 'Service Sales Order cancelled';
      await fetchList(status: _statusFilter);
  ...
  Future<bool> close(int id) async {
    ...
    try {
      await closeSSO(id);
      _successMessage = 'Service Sales Order closed';
      await fetchList(status: _statusFilter);
      if (_detail != null) await fetchDetail(_detail!.encryption);
      return true;
    } catch (e) {
  ...
  Future<bool> remove(int id) async {
    ...
    try {
      await deleteSSO(id);
      _successMessage = 'Service Sales Order deleted';
      await fetchList(status: _statusFilter);
  ...
  Future<bool> createInvoiceFromSchedule(int ssoId, int scheduleId) async {
    _isSaving = true;
    _formError = null;
    notifyListeners();
    try {
      final result = await createInvoiceFromTermUC(ssoId, scheduleId);
      _successMessage = result['message']?.toString() ?? 'Invoice created';
      if (_detail != null) await fetchDetail(_detail!.encryption);
      return true;
    } catch (e) {
