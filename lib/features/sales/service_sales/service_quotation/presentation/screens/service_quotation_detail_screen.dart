...
import 'package:erp_mobile_cnplus/features/sales/service_sales/service_quotation/presentation/widgets/service_quotation_detail_tabs.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'service_quotation_form_screen.dart';

class ServiceQuotationDetailScreen extends StatefulWidget {
  final String encryption;

  const ServiceQuotationDetailScreen({super.key, required this.encryption});

  @override
  State<ServiceQuotationDetailScreen> createState() => _State();
}

class _State extends State<ServiceQuotationDetailScreen> {
  late String _enc;
  ...
  Future<void> _load() async {
    ...
    final d = ctrl.detail;
    if (d?.isWaitingApproval == true) ctrl.loadSteps(d!.idServiceQuotation);
  }
  ...
  Future<void> _handleDelete() async {
    ...
    final success = await ctrl.remove(d.idServiceQuotation);
    if (!mounted) return;
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog('Confirm Quotation', 'Confirm this service quotation?', confirmLabel: 'Confirm');
    if (ok != true || !mounted) return;

    final f = ServiceQuotationFormModel.fromDetail(d);
  ...
  Future<void> _handleCancel() async {
    ...
    final success = await ctrl.cancel(d.idServiceQuotation, reasonCtrl.text.trim());
    if (!mounted) return;
    _snack(success ? 'Cancelled' : ctrl.formError ?? 'Failed', success: success);
  }

  Future<void> _handleCreateSSO() async {
    final ctrl = context.read<ServiceQuotationController>();
    final d = ctrl.detail;
    if (d == null) return;
    ...
    final ok = await _confirmDialog(
      ...
      'Create a Service Sales Order from this quotation?',
      confirmLabel: 'Create SSO',
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.createSSO(d.idServiceQuotation);
    if (!mounted) return;
    _snack(success ? ctrl.successMessage! : ctrl.formError ?? 'Failed', success: success);
  }
  ...
  Future<void> _handleApprove() async {
    ...
    final success = await ctrl.approve(d.idServiceQuotation);
    if (!mounted) return;
  ...
  Future<void> _handleReject() async {
    ...
    final success = await ctrl.reject(d.idServiceQuotation);
    if (!mounted) return;
  ...
  Future<void> _handleViewSteps() async {
    ...
    await ctrl.loadSteps(d.idServiceQuotation);
    if (!mounted) return;
  ...
  void _handleReturnFromForm(ServiceQuotationController ctrl) {
    if (!mounted) return;
  ...
  // Simplified to match Sales Order
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 1,
        actions: [
          Consumer<ServiceQuotationController>(
            builder: (_, ctrl, __) {
              final d = ctrl.detail;
                  ...
                  if (d.canEdit)
                    IconButton(
                      ...
                      onPressed: () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => ServiceQuotationFormScreen(encryption: _enc)),
                      ).then((_) {
                        if (!mounted) return;
                        final ne = ctrl.savedEncryption;
                        if (ne != null && ne.isNotEmpty && ne != _enc) {
          ...
          return Stack(
            children: [
              Column(
                children: [
                  Expanded(
                    child: ServiceQuotationDetailTabs(detail: d),
                  ),
                  if (d.isDraft)
                    _bottomBar([
