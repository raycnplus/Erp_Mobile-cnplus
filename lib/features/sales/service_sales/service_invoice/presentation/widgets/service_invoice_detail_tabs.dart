...
class ServiceInvoiceDetailTabs extends StatefulWidget {
  final ServiceInvoiceDetailModel detail;

  const ServiceInvoiceDetailTabs({
    super.key,
    required this.detail,
  });

  @override
  State<ServiceInvoiceDetailTabs> createState() {
    return _State();
  }
}

class _State extends State<ServiceInvoiceDetailTabs> with SingleTickerProviderStateMixin {
  ...
  Widget _buildServicesTab(ServiceInvoiceDetailModel d) {
    return Column(
      children: [
        Expanded(
  ...
  Widget _buildInfoTab(ServiceInvoiceDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          _infoCard('Service Invoice Info', Icons.receipt_long_outlined, [
            ...
            if (d.paymentScheduleName?.isNotEmpty == true) _row('Schedule Term', d.paymentScheduleName!),
          ...
          if (d.hasLinkedDoc) ...[
            ...
            _infoCard('Linked Documents', Icons.link_outlined, [
              if (d.serviceSalesOrderEncryption?.isNotEmpty == true)
                _row('Service Sales Order', d.serviceSalesOrderReference ?? 'Linked'),
              if (d.serviceDirectSalesEncryption?.isNotEmpty == true)
                _row('Service Direct Sales', d.serviceDirectSalesReference ?? 'Linked'),
            ]),
          ],
          const SizedBox(height: 12),
        ],
  ...
  Widget _buildPaymentTab(ServiceInvoiceDetailModel d) {
    if (!d.hasPayment) {
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              d.isPosted ? 'No payment recorded yet' : 'Invoice not yet posted',
              style: GoogleFonts.poppins(
                fontSize: 15,
                color: colorTextSubtle,
              ),
  ...
  Widget _buildOthersTab(ServiceInvoiceDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
