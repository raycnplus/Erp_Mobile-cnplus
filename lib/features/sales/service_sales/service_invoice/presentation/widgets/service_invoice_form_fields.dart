...
class ServiceInvoiceFormFields extends StatefulWidget {
  final ServiceInvoiceFormModel formData;
  final ServiceInvoiceFormOptions? formOptions;
  final VoidCallback onSave;
  final bool isSaving;

  const ServiceInvoiceFormFields({
    super.key,
  ...
  @override
  State<ServiceInvoiceFormFields> createState() => _State();
}

class _State extends State<ServiceInvoiceFormFields> with SingleTickerProviderStateMixin {
  ...
  ServiceInvoiceFormOptions? get _opts => widget.formOptions;
  ...
  Future<void> _addService() async {
    ...
    if (services.isEmpty) { _snack('No services available'); return; }

    SIServiceOption? selected;

    final result = await showDialog<SIServiceOption>(
      context: context,
        ...
        builder: (ctx, setSt) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                CustomSearchableDropdown<SIServiceOption>(
                  value: selected,
                  items: services,
    ...
    if (result != null && mounted) {
      setState(() {
        widget.formData.items.add(ServiceInvoiceFormItem(
  ...
  Widget _buildHeaderTab() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        CustomSearchableDropdown<SICustomerOption>(
          ...
          items: _opts?.customers ?? [], itemLabel: (c) => c.name,
          onChanged: (v) => setState(() => widget.formData.idCustomer = v?.id),
          isRequired: true, label: 'Customer', clearable: false,
        ),
        const SizedBox(height: 14),

        CustomSearchableDropdown<SISalesPersonOption>(
          ...
          items: _opts?.salesPersons ?? [], itemLabel: (s) => s.name,
          onChanged: (v) => setState(() => widget.formData.salesPerson = v?.id),
          isRequired: true, label: 'Salesperson', clearable: false,
        ),
        const SizedBox(height: 14),

        _sectionLabel('Invoice Date'),
        _datePicker(
          value: widget.formData.invoiceDate,
          hint: 'Select invoice date',
          onPicked: (d) => setState(() => widget.formData.invoiceDate = d),
        ),
        const SizedBox(height: 14),

        CustomSearchableDropdown<SIPaymentTermOption>(
        ...
        CustomFormInput(
          ...
          hintText: 'Write note for this service invoice (optional)', maxLines: 3,
        ),
      ...
      Expanded(
        child: widget.formData.items.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Icon(Icons.design_services_outlined, size: 64, color: colorGrey),
                const SizedBox(height: 12),
                Text('No services added', style: GoogleFonts.poppins(fontSize: 16, color: colorTextSubtle)),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(item.serviceName ?? 'Service ${i + 1}',
          ...
          CustomFormInput(
            ...
            label: 'Description (optional)', hintText: 'Service description',
          ...
          Row(children: [
            Expanded(child: _numField(
              ctrl: _itemCtrl(i, 'qty', item.qty), label: 'Qty', required: true,
              onChanged: (v) => setState(() { item.qty = v; _recalcItem(i); }),
            )),
  ...
  Widget _buildBottomBar() => Container(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(
              widget.formData.isEditMode ? 'Update Service Invoice' : 'Save Service Invoice',
