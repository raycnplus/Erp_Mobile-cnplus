...
import 'package:erp_mobile_cnplus/features/sales/service_sales/service_invoice/presentation/widgets/service_invoice_detail_tabs.dart';
import 'package:erp_mobile_cnplus/core/utils/colors.dart';
import 'service_invoice_form_screen.dart';

class ServiceInvoiceDetailScreen extends StatefulWidget {
  final String encryption;

  const ServiceInvoiceDetailScreen({
    super.key,
    required this.encryption,
  });

  @override
  State<ServiceInvoiceDetailScreen> createState() {
    return _State();
  }
}

class _State extends State<ServiceInvoiceDetailScreen> {
  late String _enc;
  ...
  Future<void> _load() async {
    ...
    if (d?.isWaitingApproval == true) {
      ctrl.loadSteps(d!.idServiceInvoice);
    }
  }
  ...
  Future<void> _handleDelete() async {
    ...
    final ok = await _confirmDialog(
      'Delete Draft',
      'Delete this service invoice draft?',
      confirmLabel: 'Delete',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.remove(d.idServiceInvoice);
    if (!mounted) return;
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog('Confirm Invoice', 'Confirm this service invoice?', confirmLabel: 'Confirm');
    if (ok != true || !mounted) return;

    final f = ServiceInvoiceFormModel.fromDetail(d);
  ...
  Future<void> _handlePost() async {
    ...
    if (ctrl.formOptions?.autoCreateJournal == true) {
      journalDate = await showDatePicker(
        context: context,
        initialDate: DateTime.now(),
        firstDate: DateTime(DateTime.now().year - 1),
        lastDate: DateTime(DateTime.now().year + 3),
        helpText: 'Journal Transaction Date',
        builder: (ctx, child) {
          return Theme(
    ...
    final ok = await _confirmDialog(
      'Post Invoice',
      'Post this service invoice? This will create accounting journal entries.',
      confirmLabel: 'Post',
    );
    if (ok != true || !mounted) return;

    final f = ServiceInvoiceFormModel.fromDetail(d);
    f.journalTransactionDate = journalDate;
  ...
  Future<void> _handleCancel() async {
    ...
    final success = await ctrl.cancel(d.idServiceInvoice, reasonCtrl.text.trim());
    if (!mounted) return;
  ...
  Future<void> _handleCreatePayment() async {
    ...
    final ok = await _confirmDialog('Create Payment', 'Create payment for this service invoice?', confirmLabel: 'Create');
    if (ok != true || !mounted) return;

    final success = await ctrl.createPayment(d.idServiceInvoice);
    if (!mounted) return;
    _snack(success ? ctrl.successMessage! : ctrl.formError ?? 'Failed', success: success);
  }
  ...
  Future<void> _handleApprove() async {
    ...
    final ok = await _confirmDialog(
      'Approve',
      'Approve this service invoice?',
      confirmLabel: 'Approve',
      confirmColor: const Color(0xFF2E7D32),
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.approve(d.idServiceInvoice);
    if (!mounted) return;
  ...
  Future<void> _handleReject() async {
    ...
    final ok = await _confirmDialog(
      'Reject',
      'Reject this service invoice?',
      confirmLabel: 'Reject',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.reject(d.idServiceInvoice);
    if (!mounted) return;
  ...
  Future<void> _handleViewSteps() async {
    ...
    await ctrl.loadSteps(d.idServiceInvoice);
    if (!mounted) return;
  ...
  // Sederhanakan _canApprove sama seperti Sales Order
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) {
                              return ServiceInvoiceFormScreen(encryption: _enc);
                            },
                          ),
                        ).then((_) {
                          if (!mounted) return;
          ...
          return Stack(
            children: [
              Column(
                children: [
                  Expanded(
                    child: ServiceInvoiceDetailTabs(detail: d),
                  ),
                  if (d.isDraft)
                    _bottomBar([
                  ...
                  if (d.isConfirmed)
                    _bottomBar([
                      ...
                      _primaryBtn('Post', Icons.verified_outlined, _handlePost),
                    ]),
                  if (d.isPosted && !d.hasPayment)
                    _bottomBar([
                      _primaryBtn('Create Payment', Icons.payment_outlined, _handleCreatePayment),
                    ]),
                ],
              ),
              if (ctrl.isSaving)
