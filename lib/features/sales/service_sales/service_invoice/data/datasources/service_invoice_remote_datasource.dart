...
class ServiceInvoiceRemoteDataSource {
  final Dio dio;

  ServiceInvoiceRemoteDataSource(this.dio);
  ...
  Future<Map<String, dynamic>> getList({
    ...
    try {
      final r = await dio.get('/sales/service/invoices', queryParameters: {
        'page':     page,
        'per_page': perPage,
      ...
      return {
        'items': (pd['data'] as List).map((e) => ServiceInvoiceModel.fromJson(e)).toList(),
        'meta': ServiceInvoicePaginationMeta(
  ...
  Future<ServiceInvoiceDetailModel> getDetail(String enc) async {
    try {
      final r = await dio.get('/sales/service/invoices/$enc');
      return ServiceInvoiceDetailModel.fromJson(r.data);
    } on DioException catch (e) {
      throw Exception(_err(e));
    }
  }

  Future<ServiceInvoiceFormOptions> getFormOptions() async {
    try {
      final results = await Future.wait([
        dio.get('/sales/service/invoices/form-options'),
        dio.get('/sales/service/invoices/service-list'),
      ]);
      final optData = Map<String, dynamic>.from(results[0].data['data'] ?? results[0].data);
      optData['services'] = results[1].data['services'] as List? ?? [];
      return ServiceInvoiceFormOptions.fromJson({'data': optData});
    } on DioException catch (e) {
      throw Exception(_err(e));
    }
  }
  ...
  Future<void> cancel(int id, String reason) async {
    try {
      await dio.post('/sales/service/invoices/$id/cancel', data: {'cancel_reason': reason});
    } on DioException catch (e) {
      throw Exception(_err(e));
    }
  }
  ...
  Future<void> delete(int id) async {
    try {
      await dio.delete('/sales/service/invoices/$id');
    } on DioException catch (e) {
      throw Exception(_err(e));
    }
  }
  ...
  Future<Map<String, dynamic>> createPayment(int id) async {
    try {
      final r = await dio.post('/sales/service/invoices/$id/create-payment');
  ...
  Future<double?> getPriceFromList(int productId, int priceListId) async {
    try {
      final r = await dio.get(
        '/sales/service/invoices/price-from-list',
  ...
  Future<void> approve(int id) async {
    try {
      await dio.post('/sales/service/invoices/$id/approve');
    } on DioException catch (e) {
      throw Exception(_err(e));
    }
  }
  ...
  Future<void> reject(int id) async {
    try {
      await dio.post('/sales/service/invoices/$id/reject');
    } on DioException catch (e) {
      throw Exception(_err(e));
    }
  }
  ...
  Future<Map<String, dynamic>> getSteps(int id) async {
    try {
      final r = await dio.get('/sales/service/invoices/$id/steps');
      return r.data as Map<String, dynamic>;
    } on DioException catch (e) {
      throw Exception(_err(e));
    }
