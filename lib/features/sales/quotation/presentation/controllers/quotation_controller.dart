...
class QuotationController extends ChangeNotifier {
  final GetQuotationList getList;
  final GetQuotationDetail getDetail;
  final GetQuotationFormOptions getFormOptions;
  final SaveQuotation saveQ;
  final CancelQuotation cancelQ;
  final DeleteQuotation deleteQ;
  final CreateSalesOrderFromQuotation createSOUC;
  final GetProductsByLocation getProductsByLocation;
  final GetPriceFromList getPriceFromList;
  final ApproveQuotation approveQ;
  final RejectQuotation rejectQ;
  final GetQuotationSteps getSteps;
  ...
  List<QuotationModel> _all = [], _filtered = [];
  ...
  QuotationFormOptions? _formOptions;
  bool _isLoadingOptions = false;
  bool _optionsLoaded = false;
  bool _isSaving = false;
  String? _formError, _successMessage;
  ...
  List<ProductOption> _locationProducts = [];

  List<QuotationModel> get pageItems {
  ...
  QuotationDetailModel? get detail => _detail;
  bool get isLoadingDetail  => _isLoadingDetail;
  String? get detailError   => _detailError;
  QuotationFormOptions? get formOptions => _formOptions;
  ...
  List<ProductOption> get locationProducts => _locationProducts;
  ...
  Future<void> fetchList({String? status}) async {
    ...
    try {
      ...
      while (true) {
        ...
        final meta = r['meta'] as QuotationPaginationMeta;
        all.addAll(r['items'] as List<QuotationModel>);
  ...
  void _applyFilter() {
    ...
    _filtered = _all.where((t) =>
        ...
        (t.customer?.toLowerCase().contains(q) ?? false)).toList();
  }

  void search(String q)  { _searchQuery = q.trim(); _currentPage = 1; _applyFilter(); notifyListeners(); }
  ...
  Future<List<ProductOption>> fetchProductsByLocation(int locationId) async {
  ...
  Future<bool> save(QuotationFormModel f, {String status = 'save'}) async {
    _isSaving = true;
    _formError = null;
  ...
  Future<bool> createSO(int id) async {
    _isSaving = true;
    _formError = null;
    notifyListeners();
    try {
      final result = await createSOUC(id);
      _successMessage = result['message']?.toString() ?? 'Sales Order created';
      if (_detail != null) await fetchDetail(_detail!.encryption);
      return true;
    } catch (e) {
      _formError = _msg(e);
