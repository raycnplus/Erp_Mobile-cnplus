...
class QuotationFormFields extends StatefulWidget {
  final QuotationFormModel formData;
  final QuotationFormOptions? formOptions;
  final VoidCallback onSave;
  final bool isSaving;

  const QuotationFormFields({
    super.key,
  ...
  @override
  State<QuotationFormFields> createState() => _State();
}

class _State extends State<QuotationFormFields>
  ...
  QuotationFormOptions? get _opts => widget.formOptions;
  double get _taxRate => _opts?.defaultTaxRate ?? 11.0;

  void _recalcItem(int idx) {
    final item = widget.formData.items[idx];
    item.recalculate(
  ...
  bool validate() {
    widget.formData.deliveryAddress = _deliveryCtrl.text.trim();
    ...
    if (widget.formData.sourceWarehouse == null) {
      _snack('Please select source warehouse');
      return false;
    }
    if (widget.formData.sourceLocation == null) {
      _snack('Please select source location');
      return false;
    }
    ...
    if (widget.formData.deliveryAddress.isEmpty) {
  ...
  Future<void> _addProduct() async {
    ...
    final products =
        await ctrl.fetchProductsByLocation(widget.formData.sourceLocation!);
    if (!mounted) return;
    if (products.isEmpty) {
        ...
        builder: (ctx, setSt) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                CustomSearchableDropdown<ProductOption>(
                  value: selected,
                  items: products,
                  itemLabel: (p) =>
                      '${p.productName} (${p.stockQty.toStringAsFixed(0)} on hand)',
                  onChanged: (v) => setSt(() {
                    selected = v;
                    selectedUom = null;
                  }),
                ...
                if (selected != null)
                  CustomSearchableDropdown<UomOption>(
                    value: selectedUom,
                    items: _opts?.uoms ?? [],
                    itemLabel: (u) => u.name,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.pop(
                    ctx, {'product': selected, 'uom': selectedUom});
              },
    ...
    if (result != null && mounted) {
      final prod = result['product'] as ProductOption;
      final uom = result['uom'] as UomOption;

      double unitPrice = 0;
      if (widget.formData.idPriceList != null) {
        unitPrice = await ctrl.fetchPriceFromList(
                prod.idProduct, widget.formData.idPriceList!) ??
            0;
      }

      setState(() {
        widget.formData.items.add(QuotationFormItem(
  ...
  List<LocationOption> _getFilteredLocations() {
    if (_opts == null || widget.formData.sourceWarehouse == null) return [];
    final all = _opts!.locations;
    final filtered = all
        .where((l) => l.warehouseId == widget.formData.sourceWarehouse)
        .toList();
  ...
  Widget _buildHeaderTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          CustomSearchableDropdown<CustomerOption>(
            key: ValueKey(
          ...
          CustomSearchableDropdown<WarehouseOption>(
            key: ValueKey(
                'wh_${widget.formData.sourceWarehouse}_${_opts?.warehouses.length}'),
            value: _opts?.warehouses
                .where((w) => w.id == widget.formData.sourceWarehouse)
                .firstOrNull,
          ...
          CustomSearchableDropdown<LocationOption>(
            key: ValueKey(
                'loc_${widget.formData.sourceLocation}_${widget.formData.sourceWarehouse}'),
            value: _opts?.locations
          ...
          CustomSearchableDropdown<SalesPersonOption>(
            key: ValueKey(
          ...
          CustomFormInput(
            ...
            required: true,
            validator: (_) => _deliveryCtrl.text.trim().isEmpty
                ? 'Delivery address is required'
                : null,
          ),
          const SizedBox(height: 14),

          CustomSearchableDropdown<PaymentTermOption>(
            key: ValueKey(
          ...
          CustomSearchableDropdown<PriceListOption>(
            key: ValueKey(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('No products added',
                          style: GoogleFonts.poppins(
                              fontSize: 16, color: colorTextSubtle)),
                      ...
                      ElevatedButton.icon(
                        onPressed: widget.formData.sourceLocation != null
                            ? _addProduct
                            : null,
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: OutlinedButton.icon(
                onPressed:
                    ctrl.isLoadingProducts ? null : _addProduct,
                ...
                child: Row(
                  children: [
                    const Icon(Icons.warehouse_outlined,
                        size: 12, color: colorGrey),
                    const SizedBox(width: 4),
                    Text(
                      'On Hand: ${_fmtNum(item.onHand)}',
                      style: GoogleFonts.poppins(
                        fontSize: 11,
                        color: item.onHand < item.demandQty &&
                                item.demandQty > 0
                            ? colorError
                            : colorGrey,
                      ),
            ...
            if (item.description?.isNotEmpty == true)
              Padding(
                ...
                child: Text(
                  item.description!,
                  style: GoogleFonts.poppins(
                      fontSize: 11, color: colorTextSubtle),
                  maxLines: 2,
  ...
  Widget _datePicker() => GestureDetector(
        onTap: () async {
          final picked = await showDatePicker(
            context: context,
            initialDate: widget.formData.validityDate ??
          ...
          if (picked != null) {
            setState(() => widget.formData.validityDate = picked);
          }
        },
        child: Container(
          padding:
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(10),
            border: Border.all(
              color: widget.formData.validityDate != null
                  ? colorPrimary
                  : colorGreyLight,
              width:
                  widget.formData.validityDate != null ? 1.5 : 1,
            ),
          ),
          child: Row(
            children: [
              ...
              Text(
                widget.formData.validityDate != null
                    ? DateFormat('d MMM yyyy')
                        .format(widget.formData.validityDate!)
  ...
  String _fmtNum(double v) =>
      NumberFormat('#,##0.00', 'id_ID').format(v);
}
