...
class SalesOrderDetailTabs extends StatefulWidget {
  final SalesOrderDetailModel detail;
  final VoidCallback? onCreateInvoiceFromTerm;

  const SalesOrderDetailTabs({super.key, required this.detail, this.onCreateInvoiceFromTerm});

  @override
  State<SalesOrderDetailTabs> createState() => _State();
}

class _State extends State<SalesOrderDetailTabs> with SingleTickerProviderStateMixin {
  ...
  Widget _buildProductsTab(SalesOrderDetailModel d) {
    return Column(
      children: [
        Expanded(
  ...
  Widget _buildInfoTab(SalesOrderDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          _infoCard('Sales Order Info', Icons.receipt_long_outlined, [
  ...
  Widget _buildPaymentTab(SalesOrderDetailModel d) {
    if (!d.isMultiPayment) {
      return SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _scheduleCard(SalesOrderPaymentSchedule s, SalesOrderDetailModel d) {
    Color statusColor;
    switch (s.status) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (d.isDone && s.canInvoice && widget.onCreateInvoiceFromTerm != null) ...[
  ...
  Widget _invoiceCard(SalesOrderInvoice inv) {
    return Card(
      elevation: 1,
      margin: const EdgeInsets.only(bottom: 8),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    inv.reference ?? 'Invoice #${inv.idInvoice}',
                    style: GoogleFonts.poppins(
                      fontWeight: FontWeight.w600,
                      fontSize: 13,
                      color: colorTextPrimary,
  ...
  Widget _buildOthersTab(SalesOrderDetailModel d) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
