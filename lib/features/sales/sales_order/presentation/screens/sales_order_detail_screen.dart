...
import 'package:erp_mobile_cnplus/features/sales/sales_order/presentation/widgets/sales_order_detail_tabs.dart';
...
class SalesOrderDetailScreen extends StatefulWidget {
  ...
  const SalesOrderDetailScreen({super.key, required this.encryption});
...
class _SalesOrderDetailScreenState extends State<SalesOrderDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final d = ctrl.detail;
    if (d?.isWaitingApproval == true) ctrl.loadSteps(d!.idSalesOrder);
  }
  ...
  Future<void> _handleDelete() async {
    ...
    final ok = await _confirmDialog(
      'Delete Draft',
      'Delete this sales order draft?',
      confirmLabel: 'Delete',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.remove(d.idSalesOrder);
    if (!mounted) return;
  ...
  Future<void> _handleConfirm() async {
    ...
    final ok = await _confirmDialog('Confirm Sales Order', 'Confirm this sales order?', confirmLabel: 'Confirm');
    if (ok != true || !mounted) return;
  ...
  Future<void> _handleValidate() async {
    ...
    final ok = await _confirmDialog(
      ...
      'Validate this sales order? Status will change to Done.',
  ...
  Future<void> _handleCancel() async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Cancel Sales Order', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        content: Column(
    ...
    final success = await ctrl.cancel(d.idSalesOrder, reasonCtrl.text.trim());
    if (!mounted) return;
  ...
  Future<void> _handleClose() async {
    ...
    final ok = await _confirmDialog('Close Sales Order', 'Close this sales order?', confirmLabel: 'Close');
    if (ok != true || !mounted) return;

    final success = await ctrl.close(d.idSalesOrder);
    if (!mounted) return;
  ...
  Future<void> _handleCreateInvoice() async {
    ...
    final success = await ctrl.createInvoice(d.idSalesOrder);
    if (!mounted) return;
    _snack(success ? ctrl.successMessage! : ctrl.formError ?? 'Failed', success: success);
  }
  ...
  Future<void> _handleCreateInvoiceFromTerm() async {
    ...
    SalesOrderPaymentSchedule? selected = pendingSchedules.first;

    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => StatefulBuilder(
        builder: (ctx, setSt) => AlertDialog(
          title: Text('Create Invoice from Term', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: pendingSchedules.map((s) {
              return RadioListTile<SalesOrderPaymentSchedule>(
                value: s,
                groupValue: selected,
    ...
    final success = await ctrl.createInvoiceFromSchedule(d.idSalesOrder, selected!.idPaymentSchedule);
    if (!mounted) return;
    _snack(success ? ctrl.successMessage! : ctrl.formError ?? 'Failed', success: success);
  }
  ...
  Future<void> _handleApprove() async {
    ...
    final success = await ctrl.approve(d.idSalesOrder);
    if (!mounted) return;
  ...
  Future<void> _handleReject() async {
    ...
    final ok = await _confirmDialog(
      'Reject',
      'Reject this sales order?',
      confirmLabel: 'Reject',
      confirmColor: colorError,
    );
    if (ok != true || !mounted) return;

    final success = await ctrl.reject(d.idSalesOrder);
    if (!mounted) return;
  ...
  Future<void> _handleViewSteps() async {
    ...
    await ctrl.loadSteps(d.idSalesOrder);
    if (!mounted) return;
  ...
  SalesOrderFormModel _formFromDetail(SalesOrderDetailModel d) =>SalesOrderFormModel.fromDetail(d);
                  ...
                  if (d.canEdit)
                    IconButton(
                      ...
                      onPressed: () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => SalesOrderFormScreen(encryption: _enc)),
                      ).then((_) {
                        if (!mounted) return;
          ...
          return Stack(
            children: [
              Column(
                children: [
                  Expanded(
                    child: SalesOrderDetailTabs(
                      detail: d,
                      onCreateInvoiceFromTerm:
                          d.isDone && d.isMultiPayment ? _handleCreateInvoiceFromTerm : null,
                    ),
                  ),
                  if (d.isDraft)
                    _bottomBar([
                  ...
                  if (d.isDone)
                    _bottomBar([
                      if (d.canCreateInvoice)
                        _primaryBtn('Create Invoice', Icons.receipt_outlined, _handleCreateInvoice),
                      if (d.isMultiPayment)
                        _primaryBtn('Invoice from Term', Icons.payment_outlined, _handleCreateInvoiceFromTerm),
                      if (d.canClose)
